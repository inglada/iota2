...
class Iota2TestsNumpyWorkflow(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.features_dataBase = os.path.join(
            IOTA2DIR, "data", "reduced_output_samples.sqlite"
        )
        cls.all_tests_ok = []
    ...
    def test_apply_function(self):
        ...
        custom_pipeline_array, new_labels_pip = custom_features(pipeline_array)
        self.assertTrue(
            np.allclose(np.moveaxis(custom_pipeline_array, -1, 0), test_array)
        )
    ...
    def test_sk_cross_validation(self):
        ...
        features_db_test = os.path.join(self.test_working_directory, db_file_name)
        shutil.copy(self.features_dataBase, features_db_test)

        test_model_path = os.path.join(self.test_working_directory, "test_model.rf")
        sk_learn(
        ...
        test_cross_val_results = FileSearch_AND(
            self.test_working_directory, True, "cross_val_param.cv"
        )[0]
