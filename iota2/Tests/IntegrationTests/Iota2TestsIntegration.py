...
cfg = SCF.serviceConfigFile(iota2_dataTest +
                            "/config/test_config_serviceConfigFile.cfg")
...
class iota_testFeatures(unittest.TestCase):
    ...
    def test_checkOnlySarFeatures(self):
        ...
        referenceShape_test = shapeReferenceVector(self.referenceShape,
                                                   "T31TCJ")
        preprocess("T31TCJ", config_path_test)
        tile = "T31TCJ"
        sensors_params = SCF.iota2_parameters(
            config_test).get_sensors_parameters(tile)
        #shapes genereation
        commonMasks(tile, sensors_params)
        ...
        prepare_test_selection(referenceShape_test, raster_ref, selection_test,
                               self.testPath, "code")

        TileEnvelope.GenerateShapeTile(["T31TCJ"], featuresPath,
                                       self.testPath + "/envelope", None,
                                       config_test)
        VectorSampler.generateSamples({"usually": referenceShape_test},
                                      None,
                                      config_test,
                                      sampleSelection=selection_test)
