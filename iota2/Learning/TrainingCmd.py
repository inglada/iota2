...
def writeStatsFromSample(InSamples, outStats, ground_truth, region_field):
    ...
    with open(outStats, "w") as statsFile:
        ...
        for currentStd in allStdDev:
            statsFile.write(
                '        <StatisticVector value="' + str(currentStd) + '" />\n'
            )
        statsFile.write(
            "    </Statistic>\n\
                            </FeatureStatistics>"
...
def launchTraining(cfg, dataField, stat, N, pathToCmdTrain, out, pathWd):
    ...
    samples = fu.FileSearch_AND(
        learning_directory, True, "Samples_region", "sqlite", "learn"
    )
    ...
    for sample in samples:
        ...
        if classif.lower() == "svm" or classif.lower() == "libsvm":
            outStats = os.path.join(
                outputPath, "stats", "Model_{}_seed_{}.xml".format(model, seed)
            )
