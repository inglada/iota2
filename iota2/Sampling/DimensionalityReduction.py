    ...
    if reductionMode == "global":
        ...
        for date in sorted(fd.keys()):
            ...
            for sensor in sorted(fd[date].keys()):
                tmpfl += [
                    "%s_%s_%s" % (sensor, band, date) for band in fd[date][sensor]
                ]
            fl.append(tmpfl)
        ...
        for sensor in sorted(fd.keys()):
            for band in sorted(fd[sensor].keys()):
                fl.append(
                    ["%s_%s_%s" % (sensor, band, date) for date in fd[sensor][band]]
                )
        ...
        for band in sorted(fd.keys()):
            ...
            for sensor in sorted(fd[band].keys()):
                tmpfl += [
                    "%s_%s_%s" % (sensor, band, date) for date in fd[band][sensor]
                ]
            fl.append(tmpfl)
    elif reductionMode == "sensor_date":
        ...
        for sensor in sorted(fd.keys()):
            for date in sorted(fd[sensor].keys()):
                fl.append(
                    ["%s_%s_%s" % (sensor, band, date) for band in fd[sensor][date]]
                )
    else:
...
def TrainDimensionalityReduction(
    inputSampleFileName,
    outputModelFileName,
    featureList,
    targetDimension,
...
def ExtractMetaDataFields(inputSampleFileName, reducedOutputFileName):
    ...
    cmd = 'ogr2ogr -dialect "SQLITE" -nln {} -f \'SQLite\' -select "{}" {} {}'.format(
        reducedOutputFileName_table, fields, reducedOutputFileName, inputSampleFileName
    )
    run(cmd)


def ApplyDimensionalityReduction(
    inputSampleFileName,
...
def JoinReducedSampleFiles(
    inputFileList, outputSampleFileName, component_list=None, renaming=None
):
...
def SampleFilePCAReduction(
    ...
    (featureList, MetaDataFields) = BuildFeaturesLists(
    ...
    fl_counter = 0
    inputDimensions = len(
        fu.getAllFieldsInShape(inputSampleFileName, "SQLite")[numberOfMetaDataFields:]
    )
    ...
    for fl in featureList:
        ...
        reducedSampleFile = (
            tmpDir + "/" + basename + "_reduced_" + str(fl_counter) + ".sqlite"
        )
        ...
        TrainDimensionalityReduction(
            inputSampleFileName, modelFile, fl, targetDimension, statsFile
        )
        ApplyDimensionalityReduction(
            inputSampleFileName,
            reducedSampleFile,
            modelFile,
            fl,
...
def SampleFileDimensionalityReduction(
    inSampleFile, outSampleFile, outputDir, targetDimension, reductionMode
):
...
def SampleDimensionalityReduction(
    ...
    SampleFileDimensionalityReduction(
        inSampleFile, outSampleFile, iota2_output, targetDimension, reductionMode
    )
...
def GetDimRedModelsFromClassificationModel(classificationModel, logger=logger):
    ...
    logger.debug(
        "fname : {} | outputDir : {} | region : {} | seed {}".format(
...
def ApplyDimensionalityReductionToFeatureStack(
    configFile, imageStack, dimRedModelList, logger=logger
):
...
if __name__ == "__main__":
    ...
    if args.conf:
        SampleFileDimensionalityReduction(
            args.inSampleFile, args.outSampleFile, args.conf
        )
    else:
        SampleFilePCAReduction(args.inSampleFile, args.outSampleFile, "date", 6, 5)
