...
def BuildFeaturesLists(inputSampleFileName, reductionMode='global'):
    ...
    """
    (allFeatures,
     metaDataFields) = GetAvailableFeatures(inputSampleFileName, 'global')

    fl = list()
    if reductionMode == 'global':
        fl.append(allFeatures)
    elif reductionMode == 'sensor_date':
        (fd, dummy) = GetAvailableFeatures(inputSampleFileName, 'date',
                                           'sensor')
        ...
        (fd, dummy) = GetAvailableFeatures(inputSampleFileName, 'sensor',
                                           'band')
        ...
        (fd, dummy) = GetAvailableFeatures(inputSampleFileName, 'band',
                                           'sensor')
...
def ComputeFeatureStatistics(inputSampleFileName, outputStatsFile,
                             featureList):
...
def TrainDimensionalityReduction(inputSampleFileName,
                                 outputModelFileName,
                                 featureList,
                                 targetDimension,
                                 statsFile=None):
...
def ExtractMetaDataFields(inputSampleFileName, reducedOutputFileName):
    ...
    cmd = "ogr2ogr -dialect \"SQLITE\" -nln {} -f 'SQLite' -select \"{}\" {} {}".format(
        reducedOutputFileName_table, fields, reducedOutputFileName,
        inputSampleFileName)
    run(cmd)


def ApplyDimensionalityReduction(inputSampleFileName,
...
def JoinReducedSampleFiles(inputFileList,
                           outputSampleFileName,
                           component_list=None,
                           renaming=None):
    ...
    jsq.join_sqlites(outputSampleFileName,
                     inputFileList[1:],
                     'ogc_fid',
                     component_list,
                     renaming=renaming)
...
def SampleFilePCAReduction(inputSampleFileName,
                           outputSampleFileName,
                           reductionMode,
                           targetDimension,
                           tmpDir='/tmp',
    ...
    reduced_features = [
        'reduced_' + str(pc_number) for pc_number in range(targetDimension)
    ]
    ...
    for fl in featureList:
        ...
        ApplyDimensionalityReduction(inputSampleFileName, reducedSampleFile,
                                     ...
                                     inputDimensions, statsFile)

    JoinReducedSampleFiles(reducedFileList,
                           outputSampleFileName,
                           reduced_features,
                           renaming=('reduced', targetDimension))
...
def SampleFileDimensionalityReduction(inSampleFile, outSampleFile, outputDir,
    ...
    SampleFilePCAReduction(inSampleFile,
                           outSampleFile,
                           reductionMode,
                           targetDimension,
                           reducedSamplesDir,
...
def SampleDimensionalityReduction(ioFilePair, iota2_output, targetDimension,
                                  reductionMode):
...
def BuildIOSampleFileLists(outputDir):
    ...
    for inputSampleFile in glob.glob(sampleFileDir + '/*sqlite'):
        ...
        outputSampleFile = os.path.join(reducedSamplesDir,
...
def GetDimRedModelsFromClassificationModel(classificationModel, logger=logger):
    ...
    logger.debug("fname : {} | outputDir : {} | region : {} | seed {}".format(
        fname, outputDir, region, seed))
    models = glob.glob(outputDir + '/dimRed/reduced/Samples_region_' +
...
def BuildChannelGroups(reduction_mode, output_path, logger=logger):
    ...
    """
    backupDir = os.path.join(output_path, "dimRed", "before_reduction")
    # Any original sample file will do, because we only need the names
    inputSampleFileName = glob.glob(backupDir + '/*sqlite')[0]
    (featureGroups,
     metaDataFields) = BuildFeaturesLists(inputSampleFileName, reduction_mode)
    numberOfMetaDataFields = len(metaDataFields)
    featureList = fu.getAllFieldsInShape(inputSampleFileName,
                                         'SQLite')[numberOfMetaDataFields:]
    channelGroups = list()
...
def ApplyDimensionalityReductionToFeatureStack(reduction_mode,
                                               output_path,
                                               imageStack,
                                               dimRedModelList,
                                               logger=logger):
    ...
    extractROIs = list()
    dimReds = list()
    channelGroups = BuildChannelGroups(reduction_mode, output_path)
    ...
    ConcatenateApp = OtbAppBank.CreateConcatenateImagesApplication({
        "il": dimReds,
        "out": ""
    })
...
if __name__ == "__main__":
    ...
    if args.conf:
        SampleFileDimensionalityReduction(args.inSampleFile,
                                          args.outSampleFile, args.conf)
    else:
