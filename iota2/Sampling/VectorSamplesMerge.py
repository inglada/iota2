...
def split_vectors_by_regions(path_list):
    ...
    for currentSeed in seedVector:
        regionVector = [(os.path.split(vec)[-1].split("_")
                         [regions_position], vec) for vec in currentSeed]
        regionVector_sorted_ = fu.sortByFirstElem(regionVector)
        regionVector_sorted = [
            r_vectors for region,
            r_vectors in regionVector_sorted_]
...
def check_duplicates(sqlite_file, logger=logger):
    ...
    if results:
        ...
        conn.commit()
        logger.warning(
            "{} were removed in {}".format(
                len(results), sqlite_file))
...
def vectorSamplesMerge(cfg, vectorList, logger=logger):
    ...
    seed = os.path.split(
        vectorList[0])[-1].split("_")[seed_position].replace("seed", "")

    shapeOut_name = "Samples_region_" + \
        currentModel + "_seed" + str(seed) + "_learn"
