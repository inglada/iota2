...
def getSamplesFromModelName(model_name, samplesSet, logger=logger):
    ...
    model_pos = 2
    sample = [
        path for path in samplesSet
        if os.path.basename(path).split("_")[model_pos] == model_name
    ]

    if len(sample) > 1:
        logger.error("Too many files detected in {} for the model {}".format(
            os.path.split(samplesSet[0])[0], model_name))
        raise Exception(
...
def countClassInSQLite(source_samples,
                       dataField,
                       class_name,
                       table_name="output",
                       logger=logger):
    ...
    cursor = conn.cursor()
    sql = "select * from {} where {}={}".format(table_name, dataField,
                                                class_name)
    cursor.execute(sql)
    features_number = cursor.fetchall()
    features_number = len(features_number)
    if not features_number:
        logger.warning("There is no class with the label {} in {}".format(
...
def SamplesAugmentationCounter(class_count, mode, minNumber=None,
                               byClass=None):
    ...
    if mode.lower() == "minnumber":
        ...
        max_class_count = class_count[max(class_count,
                                          key=lambda key: class_count[key])]
        for class_name, count in list(class_count.items()):
        ...
        with open(byClass, 'r') as csvfile:
            ...
            for class_name, class_samples in csv_reader:
                ...
                if class_samples > class_count[class_name]:
                    augmented_class[
                        class_name] = class_samples - class_count[class_name]
...
def DoCopy(source_samples,
           destination_samples,
           class_name,
           dataField,
           extract_quantity,
           PRIM_KEY="ogc_fid",
           source_samples_tableName="output",
           logger=logger):
    ...
    cursor.execute("CREATE TABLE tmp as select * from db_source.{}".format(
        source_samples_tableName))

    random_sql = "SELECT {}, ASTEXT(geomfromwkb(geometry, {})) FROM tmp WHERE {}={} ORDER BY RANDOM() LIMIT {}".format(
        fields, proj, dataField, class_name, extract_quantity)
    ...
    for ID_offset, sample in enumerate(samples_to_extract):
        ...
        insert = "INSERT INTO {} ({}, GEOMETRY) VALUES ({}, ST_AsBinary(ST_GeomFromText({})))".format(
            source_samples_tableName, fields, ','.join(val[:-1]), val[-1])

        try:
            logger.debug(insert)
            cursor.execute(insert)
        except:
            logger.error("failed to add the feature {} from {} to {}".format(
                old_FID, source_samples, destination_samples))
        conn.commit()
...
def DataAugmentationByCopy(dataField,
                           csv_path,
                           samplesSet,
                           workingDirectory=None,
                           PRIM_KEY="ogc_fid",
    ...
    if workingDirectory:
        ...
        for ind, sample in enumerate(samplesSet):
            ...
            samplesSet[ind] = os.path.join(workingDirectory,
    ...
    for src_model, dst_model, class_name, extract_quantity in extraction_rules:
        ...
        if extract_quantity == "-1":
            extract_quantity = countClassInSQLite(source_samples, dataField,
                                                  class_name)
        if extract_quantity == 0:
            continue

        DoCopy(source_samples, dst_samples, class_name, dataField,
               extract_quantity, PRIM_KEY, source_samples_tableName)
