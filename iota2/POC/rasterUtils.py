...
def apply_function(
    ...
    for index, roi_raster in enumerate(roi_rasters):
        ...
        (roi_array, proj_geotransform), mask = process_function(
            ...
            mask_box=(start_x, size_x, start_y, size_y),
        )
    ...
    if len(all_data_sets) > 1:
        ...
        if isinstance(new_arrays[0][0], int):
            mosaic = np.repeat(
                mask[np.newaxis, :, :], output_number_of_bands, axis=0
            )
        else:
...
def get_rasterio_datasets(
    ...
    for index, new_array in enumerate(array_proj):
        ...
        if isinstance(array, int):
            array = np.full(
                expected_arr_shape, mask_value, dtype=expected_arr_type
            )
...
def process_function(
    ...
    if roi_to_ignore is False:
        ...
        if roi_contains_mask_part:
            mask_roi_bands = np.repeat(
                mask_roi[:, :, np.newaxis], output_arr.shape[-1], axis=2
            )
