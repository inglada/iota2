...
def merge_sk_classifications(
    ...
    for element in rasters_to_merge_dic:
        ...
        merge_rasters(
            element["rasters_list"], element["merge_path"], epsg_code, working_dir
        )
...
def sk_classifications_to_merge(
    ...
    classifications = FileSearch_AND(
        iota2_classif_directory,
        True,
        "Classif",
        "_model_",
        "_seed_",
    ...
    if not len(classif_to_merge) == len(confidences_to_merge):
        raise ValueError(
            "number of classification to merge : {} is different than number of confidence to merge : {}".format(
                len(classif_to_merge), len(confidences_to_merge)
            )
        )
    for (model_name, seed_num, tile_name, suffix), classif_list in classif_to_merge:
        ...
        if suffix == "":
            classif_name = (
                "_".join(["Classif", tile_name, "model", model_name, "seed", seed_num])
                + ".tif"
            )
        else:
            classif_name = (
                "_".join(
                    ["Classif", tile_name, "model", model_name, "seed", seed_num, "SAR"]
                )
                + ".tif"
            )
        rasters_to_merge.append(
            {
                "rasters_list": classif_list,
                "merge_path": os.path.join(output_dir, classif_name),
            }
        )
    for (
        (model_name, seed_num, tile_name, suffix),
        confidence_list,
    ) in confidences_to_merge:
...
def proba_to_label(
    ...
    labels_map = np.apply_along_axis(
        func1d=get_class, axis=0, arr=proba_map, labels=labels
    )
...
def predict(
    ...
    proba_to_label(
        predicted_proba, out_classif, model.classes_, transform, epsg, masks[0]
    )
