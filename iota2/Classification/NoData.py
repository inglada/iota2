...
def concatClassifs_OneTile(
    ...
    if len(classifFusion) == 1:
        ...
        cmd = (
            "otbcli_ConcatenateImages -ram 128 -il "
            + stringClFus
            + " -out "
            + pathToDirectory
            + "/"
...
def concatRegion_OneTile(
    currentTile, pathTest, classifFusion_mask, pathWd, TileMask_concat
):
...
def noData(
    ...
    if region_vec is None:
        modelTile = gen_MaskRegionByTile(
            fieldRegion,
            Stack_ind,
            workingDir,
            currentTile,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-test.path", help="Test's path", dest="pathTest", required=True
    )
    parser.add_argument(
        "-tile.fusion.path",
