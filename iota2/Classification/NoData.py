...
def gen_MaskRegionByTile(fieldRegion, Stack_ind, workingDir, currentTile,
                         AllModel, shpRName, pathToImg, pathTest, pathWd, pathToConfig):
    modelTile = []
    for path in AllModel:
        ...
        for tile in tiles:
            ...
            maskSHP = pathTest + "/shapeRegion/" + shpRName + \
                "_region_" + model + "_" + tile + ".shp"
            maskTif = workingDir + "/" + shpRName + \
...
def concatClassifs_OneTile(pathWd, seed, currentTile,
                           pathTest, modelTile, concatOut):
...
def concatRegion_OneTile(currentTile, pathTest,
                         classifFusion_mask, pathWd, TileMask_concat):
...
def noData(pathTest, pathFusion, fieldRegion,
           pathToImg, pathToRegion, N, cfg, pathWd):
    """
    ...
    if len(modelTile) == 0 or noLabelManagement == "maxConfidence":
        ...
        cmd = "otbcli_BandMath -il " + il + " -out " + \
            imgData + ' ' + pixType + ' -exp "' + exp + '"'
        run(cmd)
        if pathWd is not None:
        ...
        # plusieurs régions) et Concaténation des masques de régions pour une
        # tuile (qui a ou non plusieurs régions)
        seed = os.path.split(pathFusion)[-1].split("_")[-1].split(".")[0]
