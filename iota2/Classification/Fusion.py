...
def dempster_shafer_fusion_parameters(iota2_dir):
    ...
    classifications = fu.FileSearch_AND(
        iota2_classif_dir, True, "Classif", ".tif")
    # group by models
    model_group = []
    for classif in classifications:
        classif_name = os.path.basename(classif)
        classif_seed = classif_name.split(
            "_")[classif_seed_pos].replace(".tif", "")
    ...
    for raster_model in raster_group:
        for raster in raster_model:
            ...
            classif_seed = raster_name.split(
                "_")[classif_seed_pos].replace(".tif", "")
            classif_model = raster_name.split("_")[classif_model_pos]
            if "SAR.tif" in raster_name:
                sar_model = fu.fileSearchRegEx(os.path.join(
                    iota2_ds_confusions_dir, "model_{}_seed_{}_SAR.csv".format(classif_model, classif_seed)))[0]
                sar_classif = raster
            else:
                opt_model = fu.fileSearchRegEx(os.path.join(
                    iota2_ds_confusions_dir, "model_{}_seed_{}.csv".format(classif_model, classif_seed)))[0]
                opt_classif = raster
...
def perform_fusion(fusion_dic, mob, classif_model_pos, classif_tile_pos, classif_seed_pos,
    ...
    model = os.path.basename(fusion_dic["sar_classif"]).split("_")[
        classif_model_pos]
    seed = os.path.basename(fusion_dic["sar_classif"]).split("_")[
        classif_seed_pos]
    tile = os.path.basename(fusion_dic["sar_classif"]).split("_")[
        classif_tile_pos]
...
def compute_fusion_choice(iota2_dir, fusion_dic, fusion_class,
    ...
    tile = os.path.basename(fusion_dic["sar_classif"]).split("_")[
        classif_tile_pos]
    im_list = [fusion_dic["sar_classif"],
               fusion_dic["opt_classif"], fusion_class]
...
def compute_confidence_fusion(fusion_dic, ds_choice,
    ...
    tile = os.path.basename(fusion_dic["sar_classif"]).split("_")[
        classif_tile_pos]
    sar_confidence = fu.fileSearchRegEx(os.path.join(
        classif_dir, "{}_model_{}_confidence_seed_{}_SAR.tif".format(tile, model, seed)))[0]
    opt_confidence = fu.fileSearchRegEx(os.path.join(
        classif_dir, "{}_model_{}_confidence_seed_{}.tif".format(tile, model, seed)))[0]
...
def compute_probamap_fusion(fusion_dic, ds_choice,
    ...
    im_list = [ds_choice, sar_proba_map,
               opt_proba_map, sar_confidence, opt_confidence]
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-path.classif", help="path to the folder which ONLY contains classification images (mandatory)", dest="pathClassif", required=True)
    parser.add_argument(
        "-conf", help="path to the configuration file which describe the classification (mandatory)", dest="pathConf", required=False)
