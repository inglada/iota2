                        ...
                        if idxband == 1:
                            ...
                            try:
                                for reg in regionprops(img, data):
                                    listlab.append(
                                        [[x for x in np.unique(reg.intensity_image) if x != 0][0], reg.area])
                            except BaseException:
                                elts, cptElts = np.unique(
                            ...
                            if len(listlab) != 0:
                                classmaj = [y for y in listlab if y[1] == max(
                                    [x[1] for x in listlab])][0][0]
                                posclassmaj = np.where(data == classmaj)
                                results = []

                                for i, g in groupby(
                                        sorted(listlab), key=lambda x: x[0]):
                                    results.append([i, sum(v[1] for v in g)])

                                sumpix = sum([x[1] for x in results])
                                for elt in [[int(w), round(
                                        ((float(z) * float(res) * float(res)) / float(sumpix)), 2)] for w, z in results]:
                                    results_final.append(
                                        [idval, 'classif', 'part'] + elt)

                        if idxband != 1:
                            if idxband == 2:
                                results_final.append([idval, 'confidence', 'mean', int(
                                    classmaj), round(np.mean(data[posclassmaj]), 2)])
                            elif idxband == 3:
                                results_final.append([idval, 'validity', 'mean', int(
                                    classmaj), round(np.mean(data[posclassmaj]), 2)])
                                results_final.append([idval, 'validity', 'std', int(
                                    classmaj), round(np.std(data[posclassmaj]), 2)])
                    else:
                        if idxband == 1:
                            results_final.append(
                                [idval, 'classif', 'part', 0, 0])
                        elif idxband == 2:
                            results_final.append(
                                [idval, 'confidence', 'mean', 0, 0])
                        elif idxband == 3:
                            results_final.append(
                                [idval, 'validity', 'mean', 0, 0])
                            results_final.append(
                                [idval, 'validity', 'std', 0, 0])
                    data = img = None
                Utils.run("rm %s" % (band))
                rastertmp = None
        else:
            results_final.append([[idval, 'classif', 'part', 0, 0], [idval, 'confidence', 'mean', 0, 0], [
                                 idval, 'validity', 'mean', 0, 0], [idval, 'validity', 'std', 0, 0]])
            print(
                "Feature with FID = %s of shapefile %s with null stats (maybe its size is too small)" %
                (idval, vector))
...
def getParameters(vectorpath, csvstorepath, chunk=1):
    ...
    if os.path.isdir(vectorpath):
        for vect in listvectors:
            ...
            csvstore = os.path.join(
                csvstorepath, "stats_%s" %
                (os.path.splitext(
                    os.path.basename(vect))[0]))
