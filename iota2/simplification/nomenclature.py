        ...
        for level in reversed(levels):
            if level != lastlevel:
                for classe in dictnomenc[level]:
                    if dictnomenc[level][classe]['code'] == dictnomenc[
                            lastlevel][cls]['parent']:
                        ...
                        for layer in symbol.findall("layer"):
                            for prop in layer.findall("prop"):
                                if prop.attrib["k"] == "color":
                                    nomenclature.append([
                                        classe[2].encode("utf-8"), classe[1],
                                        [
                                            int(x) for x in
                                            prop.attrib["v"].split(",")[0:3]
                                        ]
                                    ])

    out = [[
        x[0], x[1],
        convertRGBtoHEX(tuple(x[2])), "".join(
            str(
                unicodedata.normalize('NFKD', x[0][0:11].decode(
                    "utf-8", "ignore")).encode('ascii', 'ignore').split()))
    ] for x in nomenclature]
    for line in out:
        line[3] = ''.join(e for e in line[3] if e.isalnum())

    duplicates = [
        item for item, count in list(Counter([x[3] for x in out]).items())
        if count > 1
    ]
...
class Iota2Nomenclature():
        ...
        for ind, line in enumerate(nomen):
            ...
            for lev in range(int(self.level)):
                ...
                classeslist[ind].append(
                    (int(line[(4 * lev) + 1]), line[4 * lev],
                     line[(4 * lev) + 2], line[(4 * lev) + 3]))

        index = MultiIndex.from_tuples([tuple(x) for x in classeslist],
                                       names=levelname)
    ...
    def createRasterQML(self, outpath, classfield, level, nodata="0"):
        ...
        colorrampshader = ET.SubElement(rastershader,
                                        "colorrampshader",
        ...
        if len(self.getClass(level)) == len(self.getColor(level)):
            ...
            rasterresampler = ET.SubElement(pipe,
                                            "rasterresampler",
                                            maxOversampling="2")
