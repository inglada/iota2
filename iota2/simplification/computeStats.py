    ...
    with open(nomenclature, 'r') as f:
        for line in f.readlines():
            ...
            if int(line.split(":")[1]) != 255:
                ...
                exp2 += 'LEFT JOIN (select idstats, value from stats where info = "classif" and class = %s) out%s ON s.idstats = out%s.idstats ' % (
                    line.split(":")[1], line.split(":")[1], line.split(":")[1])
                exp3 += "CAST(%s AS NUMERIC(6,2)) AS %s, " % (line.split(":")
                                                              [2], line.split(":")[2])
...
def joinShapeStats(shapefile, stats, tmp, outfile):
    ...
    outfiletmp = os.path.join(tmp, os.path.splitext(
        os.path.basename(outfile))[0] + '_tmp.shp')
    Utils.run(
...
def computeStats(shapefile, csv, tmp, outzip=True, output=""):
    ...
    idxval = os.path.splitext(csv)[0].split(
        "_")[len(os.path.splitext(csv)[0].split("_")) - 1]
    ...
    tmpsqlite = os.path.join(
        tmp, os.path.splitext(
            os.path.basename(csv))[0] + '.sqlite')
    if os.path.exists(tmpsqlite):
        os.remove(tmpsqlite)

    importstats(csv, tmpsqlite)
    timeimport = time.time()
    logger.info(" ".join([" : ".join(["Statistics importation in sqlite database", str(
        round(timeimport - begintime, 2))]), "seconds"]))

    #pivotstats(tmpsqlite, nomenclature)
    pivotstats(tmpsqlite)

    timepivot = time.time()
    logger.info(" ".join([" : ".join(["Transpose statistics table", str(
        round(timepivot - timeimport, 2))]), "seconds"]))
    ...
    timejoin = time.time()
    logger.info(" ".join([" : ".join(["Join statistics and create final vector file", str(
        round(timejoin - timepivot, 2))]), "seconds"]))
    ...
    if outzip:
        ...
        logger.info(" ".join([" : ".join(["Compression of vector file ", str(
            round(timecompress - timejoin, 2))]), "seconds"]))
