...
def topologicalPolygonize(
    ...
    if not os.path.exists(out) and os.path.exists(raster):
        ...
        logger.info(
            'Polygonize of raster file %s' %
            (os.path.basename(raster)))
        # local environnement
        localenv = os.path.join(path, "tmp%s" %
                                (os.path.basename(os.path.splitext(raster)[0])))
        ...
        logger.info(" ".join([" : ".join(
            ["Classification raster import", str(timeimport - timeinit)]), "seconds"]))
        ...
        timevect = time.time()
        logger.info(" ".join([" : ".join(
            ["Classification vectorization", str(timevect - timeimport)]), "seconds"]))
        ...
        timeexp = time.time()
        logger.info(" ".join(
            [" : ".join(["Vectorization exportation", str(timeexp - timevect)]), "seconds"]))

        shutil.rmtree(localenv)

    else:
        logger.info(
            "Output vector %s file already exists" %
...
def generalizeVector(
    ...
    if not os.path.exists(out) and os.path.exists(vector):
        ...
        logger.info(" ".join([" : ".join(["Douglas simplification and exportation", str(
            timedouglas - timeinit)]), "seconds"]))
...
def clipVectorfile(
    ...
    if outpath == "":
        out = os.path.join(
            os.path.dirname(vector), '%s_%s.shp' %
            (prefix, str(clipvalue)))
    else:
    ...
    if not os.path.exists(out):
        ...
        timeclip = time.time()
        logger.info(" ".join(
            [" : ".join(["Clip final shapefile", str(timeclip - timeinit)]), "seconds"]))
        ...
        timeclean = time.time()
        logger.info(" ".join([" : ".join(
            ["Clean empty geometries and compute areas (ha)", str(timeclean - timedupli)]), "seconds"]))
...
def simplification(
    ...
    if douglas is not None:
        ...
        timedouglas = time.time()
        logger.info(" ".join(
            [" : ".join(["Douglas simplification", str(timedouglas - timevect)]), "seconds"]))
    ...
    if hermite is not None:
        ...
        logger.info(" ".join(
            [" : ".join(["Hermine smoothing", str(timehermine - timevect)]), "seconds"]))
    ...
    timeend = time.time()
    logger.info(" ".join([" : ".join(
        ["Global Vectorization and Simplification process", str(timeend - timeinit)]), "seconds"]))
