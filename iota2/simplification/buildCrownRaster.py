...
def manageBlocks(pathCrowns, tilenumber, blocksize,
                 inpath, outpath, ram, logger=logger):

    tabBlocks = []
    tomerge = []
    for paths, dirs, files in os.walk(pathCrowns):
        for crown in files:
            if "crown_" + str(tilenumber) + \
                    ".tif" in crown and "aux.xml" not in crown:
                shutil.copy(os.path.join(paths, crown), inpath)
                crownSource = gdal.Open(
                    os.path.join(inpath, crown), GA_ReadOnly)
                row, col = int(
                    crownSource.RasterYSize), int(
                ...
                for y in intervalY:
                    for x in intervalX:
                        outputTif = os.path.join(
                            inpath, "crown%sblock%s.tif" %
                            (tilenumber, nbblock))
                        xmin, ymin = pixToGeo(
                            os.path.join(inpath, crown), x, y)
                        xmax, ymax = pixToGeo(
                            os.path.join(
                                inpath, crown), x + blocksize, y + blocksize)

                        cmd = "gdalwarp -overwrite -multi --config GDAL_CACHEMAX 9000 -wm 9000 -wo NUM_THREADS=ALL_CPUS -te " + \
                            str(xmin) + " " + str(ymax) + " " + \
                            str(xmax) + " " + str(ymin)
                        cmd = cmd + " -ot UInt32 " + \
                            os.path.join(inpath, crown) + " " + outputTif
                ...
                fu.assembleTile_Merge(
                    tomerge, int(
                        round(
                            ds.GetGeoTransform()[1], 0)), out, ot="Byte")
                ...
                logger.info(
                    'Crown raster of tile %s is now ready' %
                    (tilenumber))
