...
def manageBlocks(
    pathCrowns, tilenumber, blocksize, inpath, outpath, ram, logger=logger
):
    ...
    for paths, dirs, files in os.walk(pathCrowns):
        for crown in files:
            if (
                ...
                with open(
                    os.path.join(pathCrowns, "listid_%s" % (tilenumber)), "rb"
                ) as f:
                ...
                for y in intervalY:
                    for x in intervalX:
                        ...
                        cmd = (
                            "gdalwarp -overwrite -multi --config GDAL_CACHEMAX 9000 -wm 9000 -wo NUM_THREADS=ALL_CPUS -te "
                            + str(xmin)
                            + " "
                            + str(ymax)
                            + " "
