...
class Sentinel_2_S2C(Sensor):
    ...
    def __init__(self, config_path, tile_name):
        ...
        if extract_bands_flag:
            ...
            self.extracted_bands = [
                ...
                if band_name in self.cfg_IOTA2.getParam("Sentinel_2_S2C", "keepBands")
            ]
    ...
    def sort_dates_directories(self, dates_directories):
        ...
        return sorted(
            dates_directories,
            key=lambda x: int(
                os.path.basename(x).split("_")[self.date_position].split("T")[0]
            ),
        )
    ...
    def preprocess_date_masks(
        ...
        binary_mask = CreateBandMathApplication(
            {"il": scl, "exp": "{}?1:0".format(invalid_expr), "pixType": "uint8"}
        )
        binary_mask.Execute()
        app_dep = [binary_mask]
