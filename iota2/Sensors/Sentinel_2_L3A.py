...
class Sentinel_2_L3A():
        ...
        if extract_bands_flag:
            ...
            self.extracted_bands = [(band_name, band_position + 1)
                                    ...
                                    if band_name in self.cfg_IOTA2.getParam(
                                        "Sentinel_2_L3A", "keepBands")]

        # about gapFilling interpolations
        self.temporal_res = self.cfg_IOTA2.getParam("Sentinel_2_L3A",
                                                    "temporalResolution")
    ...
    def write_interpolation_dates_file(self):
        ...
        if not self.cfg_IOTA2.getParam("GlobChain", "autoDate"):
            date_interp_min = self.cfg_IOTA2.getParam("Sentinel_2_L3A",
                                                      "startDate")
            date_interp_max = self.cfg_IOTA2.getParam("Sentinel_2_L3A",
                                                      "endDate")
    ...
    def get_time_series_gapFilling(self, ram=128):
        ...
        features_labels = [
            "{}_{}_{}".format(self.__class__.name, band_name, date)
            for date in dates_in for band_name in bands
        ]
    ...
    def get_features(self, ram=128, logger=logger):
        ...
        if hand_features_flag:
            hand_features = self.cfg_IOTA2.getParam("Sentinel_2_L3A",
                                                    "additionalFeatures")
            comp = len(
                self.stack_band_position) if not self.extracted_bands else len(
                    self.extracted_bands)
