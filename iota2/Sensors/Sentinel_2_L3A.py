...
class Sentinel_2_L3A(Sensor):
    ...
    def __init__(self, config_path, tile_name):
        ...
        self.write_dates_stack = self.cfg_IOTA2.getParam(
            "Sentinel_2_L3A", "write_reproject_resampled_input_dates_stack"
        )
        ...
        if extract_bands_flag:
            ...
            self.extracted_bands = [
                ...
                if band_name
                in self.cfg_IOTA2.getParam("Sentinel_2_L3A", "keepBands")
            ]
    ...
    def get_available_dates(self):
        ...
        stacks = sorted(
            FileSearch_AND(self.output_preprocess_directory, True, pattern),
    ...
    def preprocess_date(
        ...
        # get bands
        date_bands = [
            FileSearch_AND(date_dir, True, "FRC_{}.tif".format(bands_name))[0]
    ...
    def preprocess_date_masks(
        ...
        date_mask = glob.glob(
            os.path.join(
                date_dir,
                "{}{}".format(self.struct_path_masks, self.masks_pattern),
            )
        )[0]
        ...
        if int(self.target_proj) != int(mask_projection):
            ...
            ds = Warp(
                ...
                xRes=10,
                yRes=10,
                srcSRS="EPSG:{}".format(mask_projection),
                dstSRS="EPSG:{}".format(self.target_proj),
                options=["INIT_DEST=0"],
            )
            if working_dir:
    ...
    def get_time_series_masks(self, ram=128, logger=logger):
        ...
        if nb_available_dates != len(available_masks):
            error = "Available dates ({}) and avaibles masks ({}) are different".format(
                nb_available_dates, len(available_masks)
            )
            logger.error(error)
