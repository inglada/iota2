...
class Landsat_5_old(Sensor):
    ...
    def __init__(self, config_path, tile_name):
        ...
        cfg_sensors = os.path.join(get_iota2_project_dir(), "iota2", "Sensors", "sensors.cfg")
        ...
        if extract_bands_flag:
            self.extracted_bands = [(band_name, band_position + 1) for band_position, band_name in enumerate(self.stack_band_position) if band_name in self.cfg_IOTA2.getParam("Landsat5_old", "keepBands")]
        ...
        self.temporal_res = self.cfg_IOTA2.getParam("Landsat5_old", "temporalResolution")
    ...
    def write_interpolation_dates_file(self, write=True):
        ...
        date_interp_min, date_interp_max = getDateLandsat(self.l5_old_data, self.all_tiles.split(" "))
        # force dates
    ...
    def get_features(self, ram=128, logger=logger):
        ...
        if hand_features_flag:
            hand_features = self.cfg_IOTA2.getParam("Landsat5_old", "additionalFeatures")
            comp = len(self.stack_band_position) if not self.extracted_bands else len(self.extracted_bands)
