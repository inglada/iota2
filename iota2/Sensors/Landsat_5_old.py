...
class Landsat_5_old(Sensor):
    ...
    def __init__(self, config_path, tile_name):
        ...
        if extract_bands_flag:
            self.extracted_bands = [
                ...
                if band_name
                in self.cfg_IOTA2.getParam("Landsat5_old", "keepBands")
            ]
    ...
    def footprint(self, ram=128):
        ...
        expr = [
            "(im{0}b1/2==rint(im{0}b1/2))".format(i + 1)
    ...
    def get_time_series(self, ram=128):
        ...
        for date_dir in input_dates:
            l5_old_date = FileSearch_AND(
                date_dir, True, self.data_type, ".TIF"
            )[0]
            if (
                not "none"
                in self.cfg_IOTA2.getParam("coregistration", "VHRPath").lower()
    ...
    def get_time_series_masks(self, ram=128, logger=logger):
        ...
        for date_dir in input_dates:
            div_mask = glob.glob(
                os.path.join(
                    date_dir,
                    "{}{}".format(self.struct_path_masks, div_mask_patter),
                )
            )[0]
            cloud_mask = glob.glob(
                os.path.join(
                    date_dir,
                    "{}{}".format(self.struct_path_masks, cloud_mask_patter),
                )
            )[0]
            sat_mask = glob.glob(
                os.path.join(
                    date_dir,
                    "{}{}".format(self.struct_path_masks, sat_mask_patter),
                )
            )[0]
