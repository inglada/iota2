...
class Landsat_5_old(Sensor):
    ...
    def __init__(self, config_path, tile_name):
        ...
        self.masks_rules = OrderedDict(
            [("_DIV.TIF", 1), ("_NUA.TIF", 0), ("_SAT.TIF", 0)])
        self.border_pos = 0
        self.cloud_pos = 1
        self.sat_pos = 2
        ...
        self.stack_band_position = ["B1", "B2", "B3", "B4", "B5", "B6"]
        ...
        self.temporal_res = self.cfg_IOTA2.getParam(
            "Landsat5_old", "temporalResolution")
    ...
    def footprint(self, ram=128):
        ...
        expr = ["(im{0}b1/2==rint(im{0}b1/2))".format(i + 1)
        ...
        if not "none" in self.cfg_IOTA2.getParam(
            ...
            reference_raster = FileSearch_AND(
                input_dates[0], True, self.data_type, "COREG", ".TIF")[0]
    ...
    def write_dates_file(self):
        ...
        all_available_dates = [
            os.path.basename(date).split("_")[
                self.date_position] for date in input_dates_dir]
    ...
    def write_interpolation_dates_file(self):
        ...
        if not self.cfg_IOTA2.getParam("GlobChain", "autoDate"):
            date_interp_min = self.cfg_IOTA2.getParam(
                "Landsat5_old", "startDate")
            date_interp_max = self.cfg_IOTA2.getParam(
                "Landsat5_old", "endDate")
    ...
    def get_time_series(self, ram=128):
        ...
        date_data = []
        for date_dir in input_dates:
            l5_old_date = FileSearch_AND(
                date_dir, True, self.data_type, ".TIF")[0]
            if not "none" in self.cfg_IOTA2.getParam(
                    'coregistration', 'VHRPath').lower():
                l5_old_date = FileSearch_AND(
                    date_dir, True, self.data_type, "COREG", ".TIF")[0]
    ...
    def get_features(self, ram=128, logger=logger):
        ...
        if hand_features_flag:
            hand_features = self.cfg_IOTA2.getParam(
                "Landsat5_old", "additionalFeatures")
            comp = len(
                self.stack_band_position) if not self.extracted_bands else len(
                self.extracted_bands)
