...
class Landsat_5_old(Sensor):
    ...
    def __init__(self, config_path, tile_name):
        ...
        output_target_dir = self.cfg_IOTA2.getParam("chain", "L5_old_output_path")
        ...
        if extract_bands_flag:
            self.extracted_bands = [
                ...
                if band_name in self.cfg_IOTA2.getParam("Landsat5_old", "keepBands")
            ]
    ...
    def footprint(self, ram=128):
        ...
        expr = [
            "(im{0}b1/2==rint(im{0}b1/2))".format(i + 1) for i in range(len(date_edge))
        ]
        expr = "{}>0?1:0".format("+".join(expr))
        masks_rules = CreateBandMathApplication(
            {"il": date_edge, "ram": str(ram), "exp": expr}
        )
        masks_rules.Execute()
        app_dep = [masks_rules]
        ...
        if not "none" in self.cfg_IOTA2.getParam("coregistration", "VHRPath").lower():
            reference_raster = FileSearch_AND(
                input_dates[0], True, self.data_type, "COREG", ".TIF"
            )[0]
    ...
    def write_dates_file(self):
        ...
        all_available_dates = [
            os.path.basename(date).split("_")[self.date_position]
    ...
    def write_interpolation_dates_file(self):
        ...
        if not self.cfg_IOTA2.getParam("GlobChain", "autoDate"):
            date_interp_min = self.cfg_IOTA2.getParam("Landsat5_old", "startDate")
            date_interp_max = self.cfg_IOTA2.getParam("Landsat5_old", "endDate")
    ...
    def get_time_series(self, ram=128):
        ...
        for date_dir in input_dates:
            ...
            if (
                ...
                l5_old_date = FileSearch_AND(
                    date_dir, True, self.data_type, "COREG", ".TIF"
                )[0]
        ...
        dates_time_series = CreateConcatenateImagesApplication(
            {"il": date_data, "out": times_series_raster, "ram": str(ram)}
        )
    ...
    def get_time_series_masks(self, ram=128, logger=logger):
        ...
        for date_dir in input_dates:
            div_mask = glob.glob(
                os.path.join(
                    date_dir, "{}{}".format(self.struct_path_masks, div_mask_patter)
                )
            )[0]
            cloud_mask = glob.glob(
                os.path.join(
                    date_dir, "{}{}".format(self.struct_path_masks, cloud_mask_patter)
                )
            )[0]
            sat_mask = glob.glob(
                os.path.join(
                    date_dir, "{}{}".format(self.struct_path_masks, sat_mask_patter)
                )
            )[0]
            ...
            date_binary_mask = CreateBandMathApplication(
                {"il": [div_mask, cloud_mask, sat_mask], "exp": expr}
            )
        ...
        dates_time_series_mask = CreateConcatenateImagesApplication(
            {"il": date_data, "ram": str(ram), "out": times_series_mask}
        )
