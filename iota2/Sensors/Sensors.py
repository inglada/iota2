...
class Landsat5(Sensor):
    ...
    def __init__(self, path_image, opath, fconf, workRes, createFolder="Create",
        ...
        if not "none" in cfg_IOTA2.getParam("coregistration", "VHRPath").lower() and len(
                glob.glob(self.path + self.struct_path + '*COREG*' + os.path.splitext(self.imType)[1])) > 0:
        ...
        self.pathmask = self.path + \
            cfg_sensors.getParam("Landsat5", "arbomask")
        if not "none" in cfg_IOTA2.getParam("coregistration", "VHRPath").lower() and len(
                glob.glob(self.pathmask + '*COREG*' + os.path.splitext(self.imType)[1])) > 0:
        ...
        sensorEnable = (self.path is not None and len(
            self.path) > 0 and 'None' not in self.path)

        # bands definitions
        self.bands["BANDS"] = OrderedDict([(key, value) for key, value in sorted(
        ...
        if sensorEnable and cfg_IOTA2.getParam(
                "iota2FeatureExtraction", "extractBands") == True:
            self.keepBands = OrderedDict([(k, v) for k, v in list(
                self.bands["BANDS"].items()) if k in cfg_IOTA2.getParam("Landsat5", "keepBands")])
...
class Landsat8(Sensor):
    ...
    def __init__(self, path_image, opath, fconf, workRes, createFolder="Create",
        ...
        self.pathmask = self.path + \
            cfg_sensors.getParam("Landsat8", "arbomask")
        if not "none" in cfg_IOTA2.getParam("coregistration", "VHRPath").lower() and len(
                glob.glob(self.pathmask + '*COREG*' + os.path.splitext(self.imType)[1])) > 0:
            nuages = os.path.splitext(self.nuages)
            saturation = os.path.splitext(self.saturation)
        ...
        if sensorEnable and cfg_IOTA2.getParam(
                "iota2FeatureExtraction", "extractBands") == True:
            self.keepBands = OrderedDict([(k, v) for k, v in list(
                self.bands["BANDS"].items()) if k in cfg_IOTA2.getParam("Landsat8", "keepBands")])
            if cfg_IOTA2.getParam("GlobChain", "features"):
                try:
...
class Sentinel_2(Sensor):
    ...
    def __init__(self, path_image, opath, fconf, workRes, createFolder="Create",
        ...
        if os.path.exists(fconf):
            ...
            # masks
            self.pathmask = self.path + \
                cfg_sensors.getParam("Sentinel_2", "arbomask")
            ...
            self.posDate = 1
            self.native_res = int(
                cfg_sensors.getParam(
                    "Sentinel_2", "nativeRes"))
            self.pathRes = tmpPath + "/LandRes_%sm/" % workRes
            ...
            if sensorEnable and cfg_IOTA2.getParam(
                    "iota2FeatureExtraction", "extractBands") == True:
                self.keepBands = OrderedDict([(k, v) for k, v in list(
                    self.bands["BANDS"].items()) if k in cfg_IOTA2.getParam("Sentinel_2", "keepBands")])
                if cfg_IOTA2.getParam("GlobChain", "features"):
                    try:
                        self.red = list(self.keepBands.keys()).index('B4')
                    except BaseException:
            ...
            try:
                ...
                if createFolder and sensorEnable:
                    ...
                    if len(self.liste) == 0:
                        ...
                        logger.debug(
                            '[Sentinel2] Found the following images: {}'.format(
                                self.liste))
                        self.imRef = self.liste[0]
            except MonException as mess:
    ...
    def getTypeMask(self, name):
        ...
        if sensorEnable:
            logger.warning(
                "[Spot4] Invalid value for No Data Mask flag in configuration file. NoDataMask not considered")
        if createFolder and sensorEnable:
            try:
                ...
                if len(liste) == 0:
                    logger.warning(
                        '[Spot4] No valid images found in {}'.format(
                            self.path))
                else:
                    logger.debug(
...
class Sentinel_2_S2C(Sensor):
    ...
    def __init__(self, path_image, opath, fconf, workRes, createFolder="Create",
        ...
        if os.path.exists(fconf):
            ...
            self.pathmask = self.path + \
                cfg_sensors.getParam("Sentinel_2_S2C", "arbomask")
            ...
            self.bands["BANDS"] = OrderedDict([(key, value) for key, value in sorted(
                iter(list(dicoBands.items())), key=lambda k_v1: (k_v1[1], k_v1[0]))])
            self.red = self.bands["BANDS"]['B4']
            self.nir = self.bands["BANDS"]['B8']
            self.swir = self.bands["BANDS"]['B11']
            ...
            if sensorEnable and cfg_IOTA2.getParam(
                    "iota2FeatureExtraction", "extractBands") == True:
                self.keepBands = OrderedDict([(k, v) for k, v in list(self.bands["BANDS"].items(
    ...
    def getDateFromName(self, nameIm, complete_date=False):
        ...
        date = os.path.splitext(
            os.path.basename(nameIm))[0].split("_")[
            self.posDate].split("T")[0]
        if complete_date:
            date = os.path.splitext(os.path.basename(nameIm))[
                0].split("_")[self.posDate]
        return date
    ...
    def CreateBorderMask_bindings(self, opath, wMode=False):
        ...
        border_exp = " + ".join(["im{}b1".format(i + 1)
