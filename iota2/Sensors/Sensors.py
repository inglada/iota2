        ...
        if (
            ...
            self.keepBands = OrderedDict(
                [
                    ...
                    if k in cfg_IOTA2.getParam("Landsat5", "keepBands")
                ]
            )
        ...
        if (
            ...
            self.keepBands = OrderedDict(
                [
                    (k, v)
                    for k, v in list(self.bands["BANDS"].items())
                    if k in cfg_IOTA2.getParam("Landsat8", "keepBands")
                ]
            )
            if cfg_IOTA2.getParam("GlobChain", "features"):
                try:
            ...
            if (
                ...
                self.keepBands = OrderedDict(
                    [
                        (k, v)
                        for k, v in list(self.bands["BANDS"].items())
                        if k in cfg_IOTA2.getParam("Sentinel_2", "keepBands")
                    ]
                )
                if cfg_IOTA2.getParam("GlobChain", "features"):
                    try:
...
class Sentinel_2_S2C(Sensor):
    ...
    def __init__(
        ...
        if os.path.exists(fconf):
            ...
            self.addFeatures = (
                cfg_IOTA2.getParam("Sentinel_2_S2C", "additionalFeatures")
            ).split(",")
    ...
    def getDateFromName(self, nameIm, complete_date=False):
        ...
        date = (
            os.path.splitext(os.path.basename(nameIm))[0]
            .split("_")[self.posDate]
            .split("T")[0]
        )
        if complete_date:
            date = os.path.splitext(os.path.basename(nameIm))[0].split("_")[
                self.posDate
            ]
        return date
    ...
    def createMaskSeries_bindings(self, opath, maskC, wMode=False, logger=logger):
        ...
        mask_serie_common.AddImageToParameterInputImageList(
            "il", mask_serie.GetParameterOutputImage("out")
        )
