...
class Sentinel_2(Sensor):
    ...
    def __init__(self, config_path, tile_name):
        ...
        self.masks_rules = OrderedDict(
            {"CLM_R1.tif": 0, "SAT_R1.tif": 0, "EDG_R1.tif": 0}
        ...
        if extract_bands_flag:
            ...
            self.extracted_bands = [
                ...
                if band_name
                in self.cfg_IOTA2.getParam("Sentinel_2", "keepBands")
            ]
    ...
    def get_available_dates(self):
        ...
        stacks = sorted(
            FileSearch_AND(
                self.output_preprocess_directory, True, "{}".format(pattern)
            ),
            key=lambda x: os.path.basename(x)
            .split("_")[self.date_position]
            .split("-")[0],
    ...
    def preprocess_date(
        ...
        date_bands = [
            FileSearch_AND(
                date_dir, True, "{}_{}.tif".format(self.data_type, bands_name)
            )[0]
