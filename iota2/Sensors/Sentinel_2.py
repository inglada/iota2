...
class Sentinel_2(Sensor):
    ...
    def __init__(self, config_path, tile_name):
        ...
        self.write_dates_stack = self.cfg_IOTA2.getParam("Sentinel_2",
        ...
        self.masks_rules = OrderedDict(
            {"CLM_R1.tif": 0, "SAT_R1.tif": 0, "EDG_R1.tif": 0})  # 0 mean data, else noData
        self.border_pos = 2
        ...
        if extract_bands_flag:
            ...
            self.extracted_bands = [
                (band_name,
                 band_position + 1) for band_position,
                band_name in enumerate(
                    self.stack_band_position) if band_name in self.cfg_IOTA2.getParam(
                    "Sentinel_2",
                    "keepBands")]
    ...
    def get_available_dates(self):
        ...
        if not "none" in self.cfg_IOTA2.getParam(
                'coregistration', 'VHRPath').lower():
    ...
    def build_stack_date_name(self, date_dir):
        ...
        _, b2_name = os.path.split(
            FileSearch_AND(
                date_dir, True, "FRE_B2.tif")[0])
        return b2_name.replace("{}_B2.tif".format(
            self.data_type), "{}_{}.tif".format(self.data_type, self.suffix))
    ...
    def preprocess_date(self, date_dir, out_prepro, working_dir=None, ram=128,
        ...
        date_bands = [
            FileSearch_AND(
                ...
                "{}_{}.tif".format(
                    self.data_type,
                    bands_name))[0] for bands_name in self.stack_band_position]
        ...
        if self.write_dates_stack:
            ...
            if os.path.exists(out_stack):
                same_proj = int(
                    getRasterProjectionEPSG(out_stack)) == int(
                    self.target_proj)
    ...
    def preprocess_date_masks(self, date_dir, out_prepro,
        ...
        if out_prepro:
            ...
            out_mask_dir = mask_dir.replace(os.path.join(
                self.s2_data, self.tile_name), out_prepro)
        ...
        # build binary mask
        expr = "+".join(["im{}b1".format(cpt + 1)
                         for cpt in range(len(date_mask))])
        ...
        if self.write_dates_stack:
            ...
            if os.path.exists(out_mask):
                same_proj = int(
                    getRasterProjectionEPSG(out_mask)) == int(
                    self.target_proj)
        ...
        for date_dir in input_dates:
            date_edge.append(
                glob.glob(
                    os.path.join(
                        date_dir, "{}{}".format(
                            self.struct_path_masks, list(
                                self.masks_rules.keys())[
                                self.border_pos])))[0])
    ...
    def write_dates_file(self):
        ...
        all_available_dates = [
            os.path.basename(date).split("_")[
                self.date_position].split("-")[0] for date in input_dates_dir]
    ...
    def get_time_series(self, ram=128):
        ...
        features_labels = [
            "{}_{}_{}".format(
                self.__class__.name,
                band_name,
                date) for date in dates_in for band_name in self.stack_band_position]
    ...
    def extract_bands_time_series(self, dates_time_series,
        ...
        features_labels = [
            "{}_{}_{}".format(
                self.__class__.name,
                band_name,
                date) for date in dates_in for band_name,
            band_pos in extract_bands]
        channels_list = ["Channel{}".format(
            channel) for channel in channels_interest]
    ...
    def get_time_series_gapFilling(self, ram=128):
        ...
        features_labels = [
            "{}_{}_{}".format(
                self.__class__.name,
                band_name,
                date) for date in dates_interp for band_name in bands]
    ...
    def get_features(self, ram=128, logger=logger):
        ...
        if features:
            ...
            if self.extracted_bands:
                bands_avail = [
                    band_name for band_name,
                    _ in self.extracted_bands]
