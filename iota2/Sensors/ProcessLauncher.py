...
def validity(tile_name, config_path, maskOut_name,
             view_threshold, workingDirectory=None, RAM=128):
    """
    ...
    threshold_raster_out = os.path.join(
        features_dir, maskOut_name.replace(
            ".shp", ".tif"))
    ...
    removeShape(
        threshold_vector_out_tmp.replace(
            ".shp", ""), [
            ".prj", ".shp", ".dbf", ".shx"])
