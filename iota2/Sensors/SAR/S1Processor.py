...
class Sentinel1_PreProcess(object):
    def __init__(self, configFile):
        ...
        if not self.ManyProjection:
            self.referencesFolder = config.get('Processing',
                                               'ReferencesFolder')
...
def S1PreProcess(cfg, process_tile, workingDirectory=None, getFiltered=False):
    ...
    rasterList = [
        elem for elem, coordinates in S1FileManager.getS1IntersectByTile(
            tilesSet[0])
    ]
    ...
    refRaster = fut.FileSearch_AND(S1chain.referencesFolder + "/T" + tile,
    ...
    for elem in rasterList_s1aASC:
        rasterList_s1aASC_reproj.append(LaunchSARreprojection_prod(elem))
    # p.terminate()
    # p.join()
    ...
    for elem in rasterList_s1aDES:
        rasterList_s1aDES_reproj.append(LaunchSARreprojection_prod(elem))
    # p.terminate()
    # p.join()
    ...
    for elem in rasterList_s1bASC:
        rasterList_s1bASC_reproj.append(LaunchSARreprojection_prod(elem))
    # p.terminate()
    # p.join()
    ...
    for elem in rasterList_s1bDES:
        rasterList_s1bDES_reproj.append(LaunchSARreprojection_prod(elem))
    # p.terminate()
    # p.join()
