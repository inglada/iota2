...
def getDatesInOtbOutputName(otbObj):
    ...
    if isinstance(otbObj, str):
        ...
        return int(otbObj.GetParameterValue(outputParameter).split(
            "/")[-1].split("_")[4].split("t")[0])
...
def remove_old_dates(img_list, new_dates):
    ...
    img_to_outcore = []
    for img in img_list:
        img_date = os.path.basename(img).split(
            "_")[date_pos].replace(".tif", "")
        if img_date in new_dates:
            img_to_outcore.append(img)
    return img_to_outcore


def main(ortho=None, configFile=None, dates=None,
         tileName=None, logger=logger):
    ...
    for d in directories[1]:
        s1_vv_DES_scene = sorted([currentOrtho for currentOrtho in getOrtho(
            ortho, "s1(.*)" + d + "(.*)vv(.*)DES(.*)tif")], key=getDatesInOtbOutputName)
        if s1_vv_DES_scene:
        ...
        s1_vh_DES_scene = sorted([currentOrtho for currentOrtho in getOrtho(
            ortho, "s1(.*)" + d + "(.*)vh(.*)DES(.*)tif")], key=getDatesInOtbOutputName)
        if s1_vh_DES_scene:
        ...
        s1_vv_ASC_scene = sorted([currentOrtho for currentOrtho in getOrtho(
            ortho, "s1(.*)" + d + "(.*)vv(.*)ASC(.*)tif")], key=getDatesInOtbOutputName)
        if s1_vv_ASC_scene:
        ...
        s1_vh_ASC_scene = sorted([currentOrtho for currentOrtho in getOrtho(
            ortho, "s1(.*)" + d + "(.*)vh(.*)ASC(.*)tif")], key=getDatesInOtbOutputName)
        if s1_vh_ASC_scene:
