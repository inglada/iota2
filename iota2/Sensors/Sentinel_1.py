...
class Sentinel_1(Sensor):
    ...
    def footprint(self, ram=128):
        ...
        s1_border_masks = FileSearch_AND(
    ...
    def get_time_series(self, ram=128):
        ...
        for filtered, masks, interp_dates, in_dates in zip(
            ...
            sar_dates = sorted(
                getNbDateInTile(in_dates, display=False, raw_dates=True),
                key=lambda x: int(x),
            )
            labels = [
                "{}_{}_{}_{}".format(
                    self.__class__.name, orbit, polarisation, date
                ).lower()
    ...
    def get_time_series_gapFilling(self, ram=128):
        ...
        for filtered, masks, interp_dates, in_dates in zip(
            ...
            sar_dates = sorted(
                getNbDateInTile(interp_dates, display=False, raw_dates=True),
                key=lambda x: int(x),
            )
            labels = [
                "{}_{}_{}_{}".format(
    ...
    def get_features(self, ram=128, logger=logger):
        ...
        for sensor_mode, time_series_app in list(s1_data.items()):
            ...
            sar_time_series[orbit.lower()][polarisation.lower()][
                "availDates"
            ] = getNbDateInTile(date_file, display=False, raw_dates=True)
        features_labels = []
