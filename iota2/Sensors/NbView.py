...
def nbViewOptical(
    ...
    AllRefl, AllMask, datesInterp, realDates = gapFillingToSample(
        tile,
        tilesStackDirectory,
        "samples",
        "dataField",
    ...
    if not os.path.exists(tilePath + "/Final"):
        ...
        fu.updateDirectory(
            tilesStackDirectory + "/" + tile + "/Final", tilePath + "/Final"
        )

    for currentMask in AllMask:
        currentMask[0].Execute()

    concat = OtbAppBank.CreateConcatenateImagesApplication(
        {"il": AllMask, "pixType": "uint8", "out": ""}
    )
    concat.Execute()
    ...
    expr = (
        str(nbRealDates)
        + "-"
        + "-".join(["im1b" + str(band + 1) for band in range(nbRealDates)])
    )
    print(expr)
...
def nbViewOpticalAndSAR(tile, workingDirectory, cfg, outputRaster, tilePath):
    ...
    sarView.Execute()
    nbViewOpt, tilesStackDirectory, opt_ = nbViewOptical(
        tile, workingDirectory, cfg, outputRaster, tilePath
    )
    nbViewOpt.Execute()

    nbViewSarOpt = OtbAppBank.CreateBandMathApplication(
        {
            "il": [(nbViewOpt, opt_), (sarView, sar_)],
            "exp": "im1b1+im2b1",
            "ram": "2500",
            "pixType": "uint8",
            "out": outputRaster,
...
def computeNbView(tile, workingDirectory, cfg, outputRaster, tilePath):
    ...
    if not sensorList:
        ...
        nbView, tilesStackDirectory, _ = nbViewOptical(
            tile, workingDirectory, cfg, outputRaster, tilePath
        )
        nbView.ExecuteAndWriteOutput()
        return tilesStackDirectory
    elif "S1" in sensorList and (len(sensorList) > 1):
        nbViewOptSAR, tilesStackDirectory, _ = nbViewOpticalAndSAR(
            tile, workingDirectory, cfg, outputRaster, tilePath
        )
...
def genNbView(TilePath, maskOut_name, nbview, cfg, workingDirectory=None):
    ...
    if not os.path.exists(TilePath + "/" + nameNbView):
        ...
        os.remove(tmp2)
        fu.removeShape(
            maskOut_tmp.replace(".shp", ""), [".prj", ".shp", ".dbf", ".shx"]
        )
        if workingDirectory:
            shutil.copy(tilePixVal, TilePath)
            fu.cpShapeFile(
                wd + "/" + maskOut.split("/")[-1].replace(".shp", ""),
                TilePath,
                [".prj", ".shp", ".dbf", ".shx"],
                spe=True,
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-path.features",
        help="path to the folder which contains features (mandatory)",
        dest="tileMaskPath",
        required=True,
    )
    parser.add_argument(
