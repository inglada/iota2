...
def PreProcessS2_S2C(
    ...
    def reproj_raster(raster_stack, outproj, workingDirectory=None):
        ...
        if int(current_proj) != int(outproj):
            ...
            ds = Warp(
                ...
                xRes=10,
                yRes=10,
                srcSRS="EPSG:{}".format(current_proj),
                dstSRS="EPSG:{}".format(outproj),
                options=["INIT_DEST=0"],
            )
    ...
    def check_bands_dates(s2c_bands_dates, logger=logger):
        ...
        dates = [
            date
            for band, date_img in list(s2c_bands_dates.items())
            for date in list(date_img.keys())
        ]
    ...
    def stack_dates(
        ...
        for s2c_date in s2c_dates:
            ...
            concatenate_out_dir, B2_name = os.path.split(
                s2c_bands_dates["B2"][s2c_date]
            )
            concatenate_out_name = B2_name.replace("B02", "STACK").replace(
                ".jp2", ".tif"
            )
            ...
            B5 = OtbAppBank.CreateRigidTransformResampleApplication(
                {
                    "in": s2c_bands_dates["B5"][s2c_date],
                    "pixType": "int16",
            ...
            B5.Execute()
            B6 = OtbAppBank.CreateRigidTransformResampleApplication(
                {
                    "in": s2c_bands_dates["B6"][s2c_date],
                    "pixType": "int16",
                    "transform.type.id.scalex": "2",
                    "transform.type.id.scaley": "2",
                    "interpolator": "bco",
            ...
            B6.Execute()
            B7 = OtbAppBank.CreateRigidTransformResampleApplication(
                {
                    "in": s2c_bands_dates["B7"][s2c_date],
                    "pixType": "int16",
                    "transform.type.id.scalex": "2",
                    "transform.type.id.scaley": "2",
                    "interpolator": "bco",
            ...
            B7.Execute()
            B8A = OtbAppBank.CreateRigidTransformResampleApplication(
                {
                    "in": s2c_bands_dates["B8A"][s2c_date],
                    "pixType": "int16",
                    "transform.type.id.scalex": "2",
                    "transform.type.id.scaley": "2",
                    "interpolator": "bco",
            ...
            B8A.Execute()
            B11 = OtbAppBank.CreateRigidTransformResampleApplication(
                {
                    "in": s2c_bands_dates["B11"][s2c_date],
                    "pixType": "int16",
                    "transform.type.id.scalex": "2",
                    "transform.type.id.scaley": "2",
                    "interpolator": "bco",
            ...
            B11.Execute()
            B12 = OtbAppBank.CreateRigidTransformResampleApplication(
                {
                    "in": s2c_bands_dates["B12"][s2c_date],
                    "pixType": "int16",
                    "transform.type.id.scalex": "2",
                    "transform.type.id.scaley": "2",
                    "interpolator": "bco",
            ...
            concatenate.AddImageToParameterInputImageList(
                "il", B5.GetParameterOutputImage("out")
            )
            concatenate.AddImageToParameterInputImageList(
                "il", B6.GetParameterOutputImage("out")
            )
            concatenate.AddImageToParameterInputImageList(
                "il", B7.GetParameterOutputImage("out")
            )
            concatenate.AddParameterStringList("il", B8)
            concatenate.AddImageToParameterInputImageList(
                "il", B8A.GetParameterOutputImage("out")
            )
            concatenate.AddImageToParameterInputImageList(
                "il", B11.GetParameterOutputImage("out")
            )
            concatenate.AddImageToParameterInputImageList(
                "il", B12.GetParameterOutputImage("out")
            )
            ...
            if not os.path.exists(concatenate_output):
                ...
                reproj_raster(concatenate_output, outproj, workingDirectory)

    def extract_SCL_masks(
        ipathS2_S2C, outproj, s2_s2c_target_dir, tile_name, workingDirectory
    ):
        ...
        SCL_dates = fu.FileSearch_AND(
            os.path.join(ipathS2_S2C, tile_name), True, "SCL_20m.jp2"
        )
        for SCL in SCL_dates:
            ...
            if s2_s2c_target_dir:
                R10_directory = os.path.join(
                    s2_s2c_target_dir, R10_directory.replace(ipathS2_S2C, "")
                )
            ...
            border_app = OtbAppBank.CreateBandMathApplication(
                {
                    "il": SCL_10m_app,
            ...
            invalid_expr = " or ".join(
                ["im1b1=={}".format(flag) for flag in invalid_flags]
            )
            invalid_app = OtbAppBank.CreateBandMathApplication(
                {
                    "il": SCL_10m_app,
...
def resample_s2(
    raster_in, s2_dir, target_dir, tile_name, workingDirectory, logger=logger
):
    """
    ...
    resample_app = OtbAppBank.CreateRigidTransformResampleApplication(
        resample_parameters
    )
...
def reprojection_s2_mask(
    ...
    cmd = 'gdalwarp -wo INIT_DEST={} -tr {} {} -s_srs "EPSG:{}" -t_srs "EPSG:{}" {} {}'.format(
        init_raster_val,
        target_res_x,
        target_res_x,
        in_epsg,
    ...
    if os.path.exists(output_path):
        ...
        if epsg_stack != out_epsg:
            ...
            Warp(
                ...
                xRes=10,
                yRes=10,
                srcSRS="EPSG:{}".format(epsg_origin),
...
def PreProcessS2(
    ...
    for date in dates:
        ...
        s2_bands["b6"] = fu.fileSearchRegEx(
            bands_10m_dir + "/" + date + "/*FRE_B6*_10M.tif"
        )[0]
        ...
        s2_bands["b11"] = fu.fileSearchRegEx(
            bands_10m_dir + "/" + date + "/*FRE_B11*_10M.tif"
        )[0]
        s2_bands["b12"] = fu.fileSearchRegEx(
            bands_10m_dir + "/" + date + "/*FRE_B12*_10M.tif"
        )[0]
...
def generateStack(
    ...
    if ipathL5:
        ...
        if "TMPDIR" in os.environ and enable_Copy is True:
            ipathL5 = copy_inputs_sensors_data(
                folder_to_copy=ipathL5,
    ...
    if ipathL8:
        ...
        if "TMPDIR" in os.environ and enable_Copy is True:
            ipathL8 = copy_inputs_sensors_data(
                folder_to_copy=ipathL8,
                workingDirectory=os.environ["TMPDIR"],
                data_dir_name="sensors_data",
                logger=logger,
            )
    ...
    if ipathS2:
        ...
        if "TMPDIR" in os.environ and enable_Copy is True:
            ipathS2 = copy_inputs_sensors_data(
                folder_to_copy=ipathS2,
                workingDirectory=os.environ["TMPDIR"],
                data_dir_name="sensors_data",
                logger=logger,
            )
    ...
    if ipathS2_S2C:
        ...
        PreProcessS2_S2C(
            projection, ipathS2_S2C, tile, s2_s2c_target_dir, workingDirectory
        )
        ipathS2_S2C = os.path.join(ipathS2_S2C, tile)
        if "TMPDIR" in os.environ and enable_Copy is True:
            ipathS2_S2C = copy_inputs_sensors_data(
                folder_to_copy=ipathS2_S2C,
                workingDirectory=os.environ["TMPDIR"],
                data_dir_name="sensors_data",
                logger=logger,
            )
        ...
        if not os.path.exists(os.path.join(outputDirectory, "tmp")):
            try:
                ...
                logger.warning(os.path.join(outputDirectory, "tmp"))
        inputDatesS2_S2C = Sentinel2_S2C.setInputDatesFile(
            os.path.join(outputDirectory, "tmp")
        )
    ...
    borderMasks = [
        sensor.CreateBorderMask_bindings(wDir, wMode=writeOutput)
        for sensor in sensors_ask
    ]
    ...
    commonRasterMask = CreateCommonZone_bindings(
        os.path.join(outputDirectory, "tmp"), borderMasks
    )
    masksSeries = [
        sensor.createMaskSeries_bindings(
            wDir.opathT, commonRasterMask, wMode=writeOutput
        )
        for sensor in sensors_ask
    ]
    temporalSeries = [
        sensor.createSerie_bindings(wDir.opathT) for sensor in sensors_ask
    ]
    ...
    if workingDirectory:
        ...
        if outputDirectory and not os.path.exists(
            outputDirectory + "/tmp/" + os.path.split(commonRasterMask)[-1]
        ):
            shutil.copy(commonRasterMask, outputDirectory + "/tmp")
            fu.cpShapeFile(
                commonRasterMask.replace(".tif", ""),
                outputDirectory + "/tmp",
                [".prj", ".shp", ".dbf", ".shx"],
                spe=True,
            )
