...
def PreProcessS2_S2C(outproj, ipathS2_S2C, tile_name,
                     s2_s2c_target_dir, workingDirectory, logger=logger):
    ...
    def check_bands_dates(s2c_bands_dates, logger=logger):
        ...
        dates = [
            date for band, date_img in list(
                s2c_bands_dates.items()) for date in list(
                date_img.keys())]
    ...
    def stack_dates(s2c_bands_dates, outproj, s2_s2c_dir,
                    s2_s2c_target_dir, tile_name, workingDirectory):
        ...
        # get all dates
        s2c_dates = list(s2c_bands_dates[list(
            s2c_bands_dates.keys())[0]].keys())
        ...
        for s2c_date in s2c_dates:
            ...
            concatenate_out_name = B2_name.replace(
                "B02", "STACK").replace(".jp2", ".tif")
            if s2_s2c_target_dir:
                concatenate_out_dir = os.path.join(
                    s2_s2c_target_dir, concatenate_out_dir.replace(
                        s2_s2c_dir, ""))
    ...
    def extract_SCL_masks(ipathS2_S2C, outproj,
                          s2_s2c_target_dir, tile_name, workingDirectory):
        ...
        for SCL in SCL_dates:
            ...
            IMG_DATA_dir = os.path.normpath(
                R20_directory).split(os.sep)[0:-1:1]
            ...
            if s2_s2c_target_dir:
                R10_directory = os.path.join(
                    s2_s2c_target_dir, R10_directory.replace(
                        ipathS2_S2C, ""))
    ...
    for img in B5:
        s2c_bands_dates["B5"][sen2cor_s2.getDateFromName(
            img, complete_date=True)] = img
    for img in B6:
        s2c_bands_dates["B6"][sen2cor_s2.getDateFromName(
            img, complete_date=True)] = img
    for img in B7:
        s2c_bands_dates["B7"][sen2cor_s2.getDateFromName(
            img, complete_date=True)] = img
    for img in B8A:
        s2c_bands_dates["B8A"][sen2cor_s2.getDateFromName(
            img, complete_date=True)] = img
    for img in B11:
        s2c_bands_dates["B11"][sen2cor_s2.getDateFromName(
            img, complete_date=True)] = img
    for img in B12:
        s2c_bands_dates["B12"][sen2cor_s2.getDateFromName(
            img, complete_date=True)] = img
    for img in B2:
        s2c_bands_dates["B2"][sen2cor_s2.getDateFromName(
            img, complete_date=True)] = img
    for img in B3:
        s2c_bands_dates["B3"][sen2cor_s2.getDateFromName(
            img, complete_date=True)] = img
    for img in B4:
        s2c_bands_dates["B4"][sen2cor_s2.getDateFromName(
            img, complete_date=True)] = img
    for img in B8:
        s2c_bands_dates["B8"][sen2cor_s2.getDateFromName(
            img, complete_date=True)] = img
...
def reprojection_s2_mask(raster_in, s2_dir, target_dir, tile_name, out_epsg,
    ...
    if not os.path.exists(output_path):
        ...
        logger.debug(
            "reproject : {} DONE at {}".format(
                raster_in, output_path))
    return output_path
...
def PreProcessS2(config, s2_dir, tile, outRes, projOut, workingDirectory,
    ...
    for date in dates:
        ...
        s2_bands["b2"] = fu.fileSearchRegEx(
            tileFolder + "/" + date + "/*FRE_B2*.tif")[0]
        s2_bands["b3"] = fu.fileSearchRegEx(
            tileFolder + "/" + date + "/*FRE_B3*.tif")[0]
        s2_bands["b4"] = fu.fileSearchRegEx(
            tileFolder + "/" + date + "/*FRE_B4*.tif")[0]
        s2_bands["b5"] = fu.fileSearchRegEx(
            bands_10m_dir + "/" + date + "/*FRE_B5*_10M.tif")[0]
        s2_bands["b6"] = fu.fileSearchRegEx(
            bands_10m_dir + "/" + date + "/*FRE_B6*_10M.tif")[0]
        s2_bands["b7"] = fu.fileSearchRegEx(
            bands_10m_dir + "/" + date + "/*FRE_B7*_10M.tif")[0]
        s2_bands["b8"] = fu.fileSearchRegEx(
            tileFolder + "/" + date + "/*FRE_B8.tif")[0]
        s2_bands["b8a"] = fu.fileSearchRegEx(
            bands_10m_dir + "/" + date + "/*FRE_B8A*_10M.tif")[0]
        s2_bands["b11"] = fu.fileSearchRegEx(
            bands_10m_dir + "/" + date + "/*FRE_B11*_10M.tif")[0]
        s2_bands["b12"] = fu.fileSearchRegEx(
            bands_10m_dir + "/" + date + "/*FRE_B12*_10M.tif")[0]
...
def generateStack(tile, cfg, outputDirectory, writeOutput=False,
    ...
    if outputDirectory and not os.path.exists(
            outputDirectory) and not testMode:
        try:
    ...
    logger.info(
        "features generation using '%s' configuration file" %
        (cfg.pathConf))
    ...
    borderMasks = [sensor.CreateBorderMask_bindings(
        wDir, wMode=writeOutput) for sensor in sensors_ask]
    ...
    masksSeries = [
        sensor.createMaskSeries_bindings(
            wDir.opathT,
            commonRasterMask,
            wMode=writeOutput) for sensor in sensors_ask]
    temporalSeries = [
        sensor.createSerie_bindings(
            wDir.opathT) for sensor in sensors_ask]
