...
def intersectSqlites(t1,
    ...
    for field in listfieldst1:
        try:
            cursor.execute("ALTER TABLE t1 ADD COLUMN %s %s;" %
                           (field[0], field[1]))
    ...
    for field in listfieldst2:
        try:
            cursor.execute("ALTER TABLE t2 ADD COLUMN %s %s;" %
                           (field[0], field[1]))
    ...
    for namefield1 in listnamefieldst1:
        if namefield1 in duplicates:
            listnamefieldst1[listnamefieldst1.index(namefield1)] = "t1." + str(
                namefield1) + " as " + "t1_" + str(namefield1)
        else:
    ...
    for namefield2 in listnamefieldst2:
        if namefield2 in duplicates:
            listnamefieldst2[listnamefieldst2.index(namefield2)] = "t2." + str(
                namefield2) + " as " + "t2_" + str(namefield2)
        else:
    ...
    if os.path.splitext(
            os.path.basename(output))[1] == '.shp' and vectformat == 'SQLite':
    ...
    if point1 or point2:
        os.system(
            ...
            % (vectformat, layerout, output,
               os.path.join(tmp, 'tmp%s.sqlite' % (layerout)), layerout))
    else:
        os.system(
