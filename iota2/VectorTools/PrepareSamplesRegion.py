...
def get_sources(cfg):
    ...
    for classe in cfg.Nomenclature:
        if not isinstance(cfg.Nomenclature[classe].Source, str):
            key = (
                cfg.Nomenclature[classe].Source[0]
                + "_"
                + cfg.Nomenclature[classe].Source[1]
            )
        else:
...
def gestionSamplesClasse(
    ...
    if cfg.globalPath[source] == "Database":
        ...
        if not isinstance(chpvalue, list) or len(chpvalue) == 1:
            ...
            if (
                len(chpvalue[0]) == 2
            ):  # une seule source plusieurs conditions vs. plusieurs sous-sources
                indfile = 0
                outpathList = []
                path = pathList[0]
                ...
                for expression in chpvalue:
                    if expression[2][0][0] == "None":
                        ...
                        output = gestionTraitementsClasse(
                            cfg,
                            infile,
                            outfile,
                            classe,
...
def gestionFichierFinal(
    samples_shapefile_source, outfile_area, ouputPath, source, classe
):
...
def gestion_echantillons(Fileconfig, ouputPath):
    ...
    for source in sources:
        if source in cfg.globalPath or (
            source.split("_")[0] in cfg.globalPath
            and source.split("_")[1] in cfg.globalPath
        ):
                ...
                if not "_" in source:
                    ...
                    try:
                        ...
                        if (priorPath is not None) and (
                            ...
                            command = "python /home/thierion/Documents/OSO/Dev/vector_tools/IntersectionQGIS.py {} {} {}".format(
    ...
    maskToMerge = []
    outfilemergemask = (
        ouputPath + "/final/" + cfg.parameters.samplesFileName + "_masks.shp"
    )
    ...
    for keysource in samples_shapefile_source:
        outfilemerge = (
            ouputPath
            + "/final/"
            + cfg.parameters.samplesFileName
            + "_"
            + keysource
            + ".shp"
        )
        ...
        if cfg.parameters.areaThresh != "":
            ...
            if keysource in sourcesAreaThresh:
                ...
                command = "python /home/thierion/Documents/OSO/Dev/vector_tools/DifferenceQGIS.py {} {} {} {}".format(
                    outfilemerge, outgridbuff, True, outfilemergeDiff
                )
                os.system(command)
    ...
    if nbfiles != 1:
        ...
        for listInOuput in outpathList:
            command = "python /home/thierion/Documents/OSO/Dev/vector_tools/DifferenceQGIS.py {} {} {} {}".format(
                listInOuput[0], listInOuput[1], True, listInOuput[2]
            )
            os.system(command)
    ...
    if os.path.exists(outfilemergemask):
        ...
        command = "python /home/thierion/Documents/OSO/Dev/vector_tools/DifferenceQGIS.py {} {} {} {}".format(
            subfinal, outfilemergemask, True, subFinalSsReseaux
        )
        os.system(command)
    ...
    try:
        ...
        print(
            "Les échantillons de classification pour la zone {}"
            " ont été produits dans la couche {}".format(
                cfg.parameters.samplesFileName, filefinal
            )
        )
    except BaseException:
...
if __name__ == "__main__":
    if len(sys.argv) == 1:
        ...
        parser = argparse.ArgumentParser(
            description="Prepare training and \
        ...
        parser.add_argument(
            ...
            dest="outpath",
            action="store",
            help="output path to store vector files",
            required=True,
        )
