...
def connect_db(user, database, host, port, password, schema):
    try:
        connString = "PG: host=%s dbname=%s user=%s password=%s port=%s" % (
...
def get_sources(cfg):
    ...
    for classe in cfg.Nomenclature:
        if not isinstance(cfg.Nomenclature[classe].Source, str):
            key = cfg.Nomenclature[classe].Source[0] + \
                '_' + cfg.Nomenclature[classe].Source[1]
        else:
...
def gestionFields(cfg, classe, source, ss_source):
    ...
    if source in cfg.Nomenclature[classe].Source:
        ...
        if isinstance(cfg.Nomenclature[classe].Champs, config.Sequence):
            ...
            chpValue.append([cfg.Nomenclature[classe].Champs,
...
def gestionTraitementsClasse(cfg, layer, outfile, classe, ss_classe, source, res, area_thresh, pix_thresh,
    ...
    if ss_classe not in cfg.parameters.maskLineaire:
        ...
        if os.path.exists(outfile):
            ...
            if typeGeom in [2, 5, -2147483643, -
                            2147483646] and buff is not None:
                outfile_buffline = outfile_ssdb[:-4] + \
                    'buffline' + str(buff) + '.shp'
            ...
            # wkbPolygon/wkbMultiPolygon/wkbPolygon25D/wkbMultiPolygon25D
                ...
                outfile_buffsurf_tmp = outfile_ssdb[:-4] + \
                    'buffsurf' + str(buff) + '_tmp.shp'
                outfile_buffsurf = outfile_ssdb[:-4] + \
                    'buffsurf' + str(buff) + '.shp'
            ...
            outfile_area = outfile_spoly[:-4] + \
                'sup' + str(pix_thresh) + 'pix.shp'
            ...
            print(
                'Aucun échantillon pour la classe {} dans la base de données {}'.format(
                    classe, source))
            return None
        ...
        if typeGeom in [2, 5, -2147483643, -2147483646] and buff is not None:
            outfile_buff_mask = outfile[:-4] + \
                'buffline' + str(buff) + '_mask.shp'
...
def gestionSamplesClasse(cfg, classe, source, ouputPath,
                         res, area_thresh, pix_thresh, lineBuffer=None):
        ...
        if not isinstance(chpvalue, list) or len(chpvalue) == 1:
            if chpvalue[0][0] == 'None':
                ...
                for path in pathList:
                    # Outfile names
                    if len(pathList) > 1:
                        outfile = ouputPath + '/' + sourcedb + '_' + \
                            classe + '_' + str(indfile) + '.shp'
                    else:
            ...
            if len(
                    chpvalue[0]) == 2:  # une seule source plusieurs conditions vs. plusieurs sous-sources
                indfile = 0
                outpathList = []
                path = pathList[0]
                ...
                outpathList[len(outpathList) - 1] = ouputPath + \
                    '/' + sourcedb + '_' + classe + '.shp'
                nbfile = 0
...
def clipFile(cfg, ouputPath, source):
    ...
    if fileToCut.lower() != 'database':
        ...
        command = "ogr2ogr -clipsrc {} {} {}".format(
            cfg.globalPath.cutFile, outputCut, fileToCut)
        os.system(command)
    else:
        print("Clip of a database layer will be managed after database extraction")


def gestionFichierFinal(samples_shapefile_source,
                        outfile_area, ouputPath, source, classe):
    if outfile_area is not None:
        if not isinstance(outfile_area, list):
            if 'mask' not in outfile_area:
                outpathfinal = '/'.join([ouputPath, 'final']) + \
                    '/' + source + '_' + classe + '.shp'
...
def gestion_echantillons(Fileconfig, ouputPath):
    ...
    for source in sources:
        if source in cfg.globalPath or (source.split(
                '_')[0] in cfg.globalPath and source.split('_')[1] in cfg.globalPath):
            for classe in sources[source]:
                ...
                if not '_' in source:
                    print(
                        'Traitement de la base de données {} pour la classe {}'.format(
                            source, classe))

                    if buff:
                        outfile_area = gestionSamplesClasse(
                            cfg, classe, source, ouputPath, res, area_thresh, pix_thresh, Buffer)
                    else:
                        outfile_area = gestionSamplesClasse(
                    ...
                    for sourceBD in source.split('_'):
                        ...
                        try:
                            Buffer = cfg.Nomenclature[classe].Buffer[source.split(
                                '_').index(sourceBD)]
                        except BaseException:
                            Buffer = None
                        ...
                        if buff and Buffer != 'None':
                            outfile_area = gestionSamplesClasse(
                                cfg, classe, sourceBD, ouputPath, res, area_thresh, pix_thresh, Buffer)
                        else:
                            outfile_area = gestionSamplesClasse(
                                cfg, classe, sourceBD, ouputPath, res, area_thresh, pix_thresh)
                    ...
                    try:
                        ...
                        if len(
                                [x for x in complexDataSets if priorSource == x[0]]) != 0:
                            priorPath = [
                                x for x in complexDataSets if priorSource == x[0]][0][1]
                            secondPath = [
                                x for x in complexDataSets if priorSource != x[0]][0][1]
                            secondSource = [
                                x for x in complexDataSets if priorSource != x[0]][0][0]
                            if cfg.parameters.landCoverField not in vf.getFields(
                                    priorPath):
                                print(
                                    'No landcover field in {} data source'.format(priorSource))
                        else:
                            print(
                        ...
                        if (priorPath is not None) and (
                            ...
                            intersectFilename = ouputPath + '/inter_' + priorSource + \
                                '_' + secondSource + '_' + classe + '.shp'
                            ...
                            command = 'python /home/thierion/Documents/OSO/Dev/vector_tools/IntersectionQGIS.py {} {} {}'.\
                                      format(
                                          priorPath, secondPath, intersectFilename)
                            os.system(command)
                        else:
            ...
            print(
                "No Path for source {} provided while required for classes {}".format(
    ...
    maskToMerge = []
    outfilemergemask = ouputPath + '/final/' + \
        cfg.parameters.samplesFileName + '_masks.shp'

    for keysource in samples_shapefile_source:
        outfilemerge = ouputPath + '/final/' + \
            cfg.parameters.samplesFileName + '_' + keysource + '.shp'

        # séparer les couches linéaire de masquage / les couches inexistantes
        # (pas d'échantillons)
        listToMerge = []
        ...
        if cfg.parameters.areaThresh != '':
            ...
            if keysource in sourcesAreaThresh:
                ...
                outfilemergeDiff = outfilemerge[:-4] + \
                    'grid_{}ha.shp'.format(cfg.parameters.areaThresh)
        ...
        # tri des chemins en fonction des priorités pour les opérations de
        # différence
        ...
        while indfile < nbfiles - 1:
            if indfile == 0:
                output = orderedSourcesPaths[indfile][:-4] + '_' + \
                    os.path.basename(
                        orderedSourcesPaths[indfile + 1]).split('_')[1][:-4] + '.shp'
                outputmerge = orderedSourcesPaths[indfile][:-4] + '_' +  \
                    os.path.basename(
                        orderedSourcesPaths[indfile + 1]).split('_')[1][:-4] + '_merge.shp'
                outpathList.append(
                    [orderedSourcesPaths[indfile], orderedSourcesPaths[indfile + 1], output, outputmerge])
            else:
                ...
                outputmerge = outpathList[indfile - 1][3][:-10] + '_' + \
                    os.path.basename(
                        orderedSourcesPaths[indfile + 1]).split('_')[1][:-4] + '_merge.shp'
                outpathList.append([outpathList[indfile -
                                                1][3], orderedSourcesPaths[indfile +
                                                                           1], output, outputmerge])
    ...
    try:
        filefinal = ouputPath + '/final/echantillons_OSO_' + \
            cfg.parameters.samplesFileName + '.shp'
