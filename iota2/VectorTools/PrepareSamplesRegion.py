...
from iota2.VectorTools import FileByClass
from iota2.VectorTools import AddFieldID
from iota2.VectorTools import AddField
from iota2.VectorTools import AddFieldArea
from iota2.VectorTools import DeleteField
from iota2.VectorTools import DeleteDuplicateGeometries
...
from iota2.VectorTools import MergeFiles
from iota2.VectorTools import CreateGrid
...
def get_sources(cfg):
    ...
    for classe in cfg.Nomenclature:
        if not isinstance(cfg.Nomenclature[classe].Source, str):
            key = cfg.Nomenclature[classe].Source[0] + '_' + cfg.Nomenclature[
                classe].Source[1]
        else:
            ...
            if isinstance(typesources, config.Sequence):
                ...
                for typesource in typesources:
                    ...
                    if isinstance(filesources, config.Sequence):
                        ...
                        multival = True
                        pathList.append(cfg.globalPath[source] + '/')
                        chpValue = gestionFields(cfg, classe, source,
                                                 typesource)
...
def gestionSamplesClasse(cfg,
    ...
    if (cfg.globalPath[source] == 'Database'):
        ...
        lyr = conn.GetLayer('{}.{}'.format(cfg.DataBase[source].schema,
        ...
        # Creation du fichier
        out = gestionTraitementsClasse(cfg, layer, outfile, classe, None,
                                       source, res, area_thresh, pix_thresh,
                                       field, values)
        ...
        if type(chpvalue) != list or len(chpvalue) == 1:
            if chpvalue[0][0] == 'None':
                ...
                for path in pathList:
                    # Outfile names
                    if len(pathList) > 1:
                        outfile = ouputPath + '/' + sourcedb + '_' + classe + '_' + str(
                            indfile) + '.shp'
                    else:
                        outfile = ouputPath + '/' + sourcedb + '_' + classe + '.shp'

                    vf.copyShapefile(path, outfile)
                    out = gestionTraitementsClasse(cfg, path, outfile, classe,
                                                   None, source, res,
                                                   area_thresh, pix_thresh,
                                                   None, None, lineBuffer)
            ...
            if len(
                    chpvalue[0]
            ) == 2:  # une seule source plusieurs conditions vs. plusieurs sous-sources
                indfile = 0
                outpathList = []
                path = pathList[0]
                ...
                outpathList[
                    len(outpathList) -
                    1] = ouputPath + '/' + sourcedb + '_' + classe + '.shp'
                nbfile = 0
                while nbfile < indfile:
                    ...
                    #value = value.replace("'","''")
                    FileByClass.FileByClass(path, chp, value,
                                            outpathList[nbfile])
...
def clipFile(cfg, ouputPath, source):
    ...
    if fileToCut.lower() != 'database':
        ...
        command = "ogr2ogr -clipsrc {} {} {}".format(cfg.globalPath.cutFile,
                                                     outputCut, fileToCut)
        os.system(command)
    else:
        print(
...
def gestionFichierFinal(samples_shapefile_source, outfile_area, ouputPath,
                        source, classe):
            ...
            for classe in sources[source]:
                ...
                if not '_' in source:
                    ...
                    try:
                        ...
                        if (priorPath is not None) and (secondPath is
                                                        not None):
