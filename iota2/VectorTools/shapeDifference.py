...
def diffBetweenLayers(
    ...
    while feature1:
        ...
        valuesfields = [
            (x, y, z) for x, y, z in valuesfields1 if y in [
                tok[2] for tok in listfieldstokeep]] + [
            (x, y, z) for x, y, z in valuesfields2 if y in [
                tok[2] for tok in listfieldstokeep]]
    ...
    for fid2 in range(0, layer2.GetFeatureCount()):
        ...
        for fid1 in list(index.intersection((xmin, xmax, ymin, ymax))):
            ...
            if field_name is not None and field_name != "":
                if (geom2.Intersects(geom1)) and (feat1.GetField(
                        field_name) != feat2.GetField(field_name)):
                    if newgeom is None:
                        continue
                    if operation == "intersection":
