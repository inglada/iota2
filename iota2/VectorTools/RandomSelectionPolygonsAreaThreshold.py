...
def get_randomPolyAreaThresh(wd, shapefile, field, classe, thresh,
                             outlistfid="", split=1, outShapefile=None, nolistid=None):
    ...
    fieldType = inLayerDefn.GetFieldDefn(
        indfield).GetFieldTypeName(fieldTypeCode)
    ...
    if outShapefile is not None:
        ...
        os.system(
            'ogr2ogr -preserve_fid -f "SQLite" %s %s' %
            (tmpsqlite, shapefile))
        ...
        for subFID in sub_FID_sqlite:
            subFid_clause.append("(ogc_fid not in ({}))".format(
                ", ".join(map(str, subFID))))
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-nolistid",
        dest="nolistid",
        help="list of field's values to not select (text file with values separated with comma)")
    parser.add_argument(
        "-outlist",
        dest="outputlist",
        help="Output file for fid list storage")
    parser.add_argument(
        "-split",
        dest="split",
        help="Split output shapefile storage",
        default=1,
        type=int)
    args = parser.parse_args()

    get_randomPolyAreaThresh(
        args.path,
        args.shape,
        args.field,
        args.classe,
