...
def get_randomPolyAreaThresh(
    ...
    if outShapefile is not None:
        ...
        for subFID in sub_FID_sqlite:
            subFid_clause.append(
                "(ogc_fid not in ({}))".format(", ".join(map(str, subFID)))
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-outlist", dest="outputlist", help="Output file for fid list storage"
    )
    parser.add_argument(
        "-split",
        dest="split",
