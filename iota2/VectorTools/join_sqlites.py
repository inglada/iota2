...
def build_fields_to_select(base_fields, fieldsnames,
                           dfield, renaming, renaming_index):
    ...
    if renaming is not None:
        ...
        fields_as += [fn + ' AS ' + fn + '_' +
                      str(renaming_index) for fn in fieldsnames]
        final_fields = ['datatojoin.' + fn + '_' +
                        str(renaming_index) for fn in fieldsnames]
...
def join_sqlites(basefile, sqlites, ofield, fieldsnames=None, dfield=None,
    ...
    for (filesqlite, fid) in zip(sqlites, list(range(len(sqlites)))):
        if os.path.exists(filesqlite):
            ...
            cursor.execute(
                "ALTER TABLE datajoin RENAME TO [%s];" %
                (tablebase))
