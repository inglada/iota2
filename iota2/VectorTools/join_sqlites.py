...
def build_fields_to_select(base_fields, fieldsnames, dfield, renaming, renaming_index):
    ...
    if renaming is not None:
        ...
        fields_as += [
            fn
            + " AS "
            + rename_field(fn, rename_pat, rename_off + idx + renaming_index)
...
def join_sqlites(
    basefile, sqlites, ofield, fieldsnames=None, dfield=None, renaming=None
):
...
if __name__ == "__main__":
    if len(sys.argv) == 1:
        ...
        PARSER.add_argument(
            ...
            dest="ofield",
            action="store",
            help="field name of base file to join tables ()",
            required=True,
        )
        PARSER.add_argument(
            "-dfield",
            dest="dfield",
            action="store",
            help="field name of joined files to join tables ()",
            required=True,
        )
        PARSER.add_argument(
            "-sqlites",
            dest="sqlites",
            nargs="+",
            help="List of sqlite files to join",
            required=True,
        )
        PARSER.add_argument(
            "-fields.names",
