...
class learnModel(IOTA2Step.Step):
    def __init__(self, cfg, cfg_resources_file, workingDirectory=None):
        ...
        self.tiles = (
            SCF.serviceConfigFile(self.cfg).getParam("chain", "listTile").split("_")
        )
        self.available_ram = 1024.0 * get_RAM(self.resources["ram"])
        self.use_scikitlearn = (
    ...
    def step_inputs(self):
        ...
        pathToModelConfig = os.path.join(
            self.output_path, "config_model", "configModel.cfg"
        )
        ...
        if self.use_scikitlearn is True and self.enable_autoContext is True:
            ...
            parameters = train_autoContext_parameters(
                self.output_path, self.region_field
            )
        else:
    ...
    def step_execute(self):
        ...
        if self.use_scikitlearn is True and self.enable_autoContext is False:
            model_parameters = SCF.serviceConfigFile(self.cfg).getSection(
