...
class Regularization(IOTA2Step.Step):
            ...
            if rastclass is None:
                if self.seed is not None:
                    ...
                    if os.path.exists(
                        ...
                        rastclass = os.path.join(self.outputPath, 'final',
                                                 'Classifications_fusion.tif')
                    else:
                        rastclass = os.path.join(self.outputPath, 'final',
                                                 'Classif_Seed_0.tif')
    ...
    def step_execute(self):
        ...
        step_function = lambda x: mr.adaptRegularization(
            self.tmpdir, x[0], os.path.join(self.outtmpdir, x[1][2]),
            str(self.RAM), x[1], self.umc)
