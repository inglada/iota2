    ...
    def checkConfigParameters(self):
        ...
        def check_sampleAugmentation():
            ...
            def check_parameters(sampleAug):
                ...
                not_allowed_p = ["in", "out", "field", "layer",
                                 "label", "seed", "inxml", "progress", "help"]
                for p in not_allowed_p:
                    if p in sampleAug:
                        raise sErr.configError(
                            "'{}' parameter must not be set in argTrain.sampleAugmentation".format(p))
        ...
        def check_sampleSelection():
            ...
            def check_parameters(sampleSel):
                ...
                for p in not_allowed_p:
                    if p in sampleSel:
                        raise sErr.configError(
                            "'{}' parameter must not be set in argTrain.sampleSelection".format(p))
