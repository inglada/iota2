...
def extractStats(vectorIn, pathConf, wD=None):
    ...
    try:
        refImg = fut.FileSearch_AND(
            iota2Folder + "/final/TMP", True, tileToCompute, ".tif"
        )[0]
    except BaseException:
        raise Exception(
            "reference image can not be found in " + iota2Folder + "/final/TMP"
        )
    ...
    stats = otbApp.CreatePolygonClassStatisticsApplication(
        {"in": refImg, "vec": vectorIn, "out": statsFile, "field": dataField}
    )
    ...
    sampleS = otbApp.CreateSampleSelectionApplication(
        {
            "in": refImg,
            "vec": vectorIn,
            "out": selVector,
    ...
    extraction = otbApp.CreateSampleExtractionApplication(
        {
            "in": dataStack,
            "vec": selVector,
            "field": dataField,
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="from a shape containing polygons,\
