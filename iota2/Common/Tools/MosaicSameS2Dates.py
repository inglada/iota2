    ...
    for currentSameDate in sameDateFolder:
        ...
        for currentBand in S2Bands:
            ...
            for currentFolder in currentSameDate:
                ...
                # -> origin tile name in raster to fit
                t = path.split("/")[-1].split("_")[-1].replace(".tif", "")
                # -> current date in raster to fit
                currentDate = path.split("/")[-1].split("_")[1].split("-")[0]
                # destination tile Name in raster name
                currentDestTile = path.split("/")[-1].split("_")[3]
