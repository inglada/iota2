...
def prepareAnnualFeatures(workingDirectory, referenceDirectory,
    ...
    for dirname, dirnames, filenames in os.walk(referenceDirectory):
        ...
        for subdirname in dirnames:
            os.mkdir(os.path.join(dirname, subdirname).replace(
                referenceDirectory, workingDirectory).replace(rename[0], rename[1]))

        # print path to all filenames.
        for filename in filenames:
            shutil.copy(os.path.join(dirname, filename), os.path.join(dirname, filename).replace(
                referenceDirectory, workingDirectory).replace(rename[0], rename[1]))
...
class iota_test_Basic(unittest.TestCase):
    ...
    def test_Basic(self):
        ...
        test_vector = fut.fileSearchRegEx(
            self.test_working_directory + "/learningSamples/*sqlite")[0]
    ...
    def test_Basic_CropMix(self):
        ...
        ref_path = os.path.join(self.iota2_directory, "data", "references",
                                "iota2tests_features_labels_test_Basic.txt")
        non_annual_features = os.path.join(
            self.test_working_directory_tmp, "non_annual_features")
        annual_features = os.path.join(
            self.test_working_directory_tmp, "annual_features")
        ...
        annual_config_path = os.path.join(
            self.test_working_directory_tmp, "AnnualConfig.cfg")
    ...
    def test_Basic_CropMix_classif(self):
        ...
        fut.cpShapeFile(vector_file.replace(".shp", ""), test_vector.replace(
            ".shp", ""), [".prj", ".shp", ".dbf", ".shx"])
        ...
        self.config.setParam(
            'argTrain', 'annualClassesExtractionSource', classifications_path)
        self.config.setParam('GlobChain', 'useAdditionalFeatures', False)

        # shapes genereation
        fut.getCommonMasks("D0005H0002", self.config, None)
        env.GenerateShapeTile(["D0005H0002"], self.test_working_directory_tmp,
                              self.test_working_directory + "/envelope", None, self.config)
        shapeRegion = os.path.join(
            self.test_working_directory_tmp, "MyFakeRegion.shp")
        area.generateRegionShape("one_region", self.test_working_directory +
                                 "/envelope", "", shapeRegion, "region", self.config, None)
        RT.createRegionsByTiles(shapeRegion, "region", self.test_working_directory +
                                "/envelope", self.test_working_directory + "/shapeRegion/", None)
