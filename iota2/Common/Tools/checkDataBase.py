...
def get_geometries_by_area(input_vector: str, area:float, driver_name: Optional[str] = "ESRI Shapefile") -> List:
...
def check_region_shape(input_vector, output_vector, field, epsg, do_corrections, display=False):
    ...
    from VectorTools.vector_functions import get_vector_proj
    ...
    if name_check is False:
        error_msg = "{} file's name not correct, it must start with an ascii letter".format(input_vector)
    ...
    shape_no_empty, empty_geom_number = checkEmptyGeom(input_vector,
    ...
    if invalid_geom != 0:
        error_msg = "'{}' contains {} invalid geometries".format(input_vector, invalid_geom)
        errors.append(ServiceError.invalidGeometry(error_msg))

    nb_too_small_geoms = len(get_geometries_by_area(input_vector, area=area_threshold, driver_name="ESRI Shapefile"))
    if nb_too_small_geoms != 0:
        errors.append(ServiceError.tooSmallRegion(input_vector, area_threshold, nb_too_small_geoms))
    return errors

def check_ground_truth(input_vector, output_vector, data_field, epsg, do_corrections, display=False):
    """
    """
    import os
