...
def addTileToGrid(UL, UR, LR, LL, newLayer, newLayerDef, tileNameField,
                  TileName):
...
def generateGridBasedSubsets(in_vec, tile, gridSize, epsg=None):
    ...
    for i in range(len(rect)):
        out_list.append(
            os.path.join(
                os.path.dirname(in_vec), tile,
                os.path.splitext(os.path.basename(in_vec))[0] + '_' +
                str(i + 1).zfill(NZ) + '.shp'))
...
def generateSegVectorTiles(in_vec, tiles, env_dir, seg_dir, epsg=None):
    from iota2.VectorTools.vector_functions import cloneVectorDataStructure
    ...
    for tile in tiles:
        ...
        ds_out.append(
            cloneVectorDataStructure(ds_in, tiles_name_out, epsg=epsg))
...
def generateSegVectorTilesRegion(in_vec, tile, tileRegs, wd, epsg=None):
    ...
    for i, region_path in enumerate(tileRegs):
        ...
        region_name_out = os.path.join(
            wd, tile + "_region_" + reg_ind + "_seg.shp")
        list_out.append(os.path.basename(region_name_out))
        ds_out.append(
            cloneVectorDataStructure(ds_in, region_name_out, epsg=epsg))
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description=
        "Change projection of a set of datas according to a spatial grid")

    parser.add_argument("-in.tileFolder",
                        dest="TileFolder",
                        help="",
                        default=None,
                        required=True)
    parser.add_argument("-in.sensorName",
                        dest="sensorName",
                        help="",
                        default=None,
                        required=True)
    parser.add_argument("-in.rasterPatterns",
                        dest="rasterPatterns",
                        help="",
                        nargs='+',
                        default=None,
                        required=True)
    parser.add_argument("-in.rasterInitValue",
                        dest="rasterInitValue",
                        help="",
                        default=None,
                        required=True)
    parser.add_argument("-in.arboRaster",
                        dest="arboRaster",
                        help="",
                        default=None,
                        required=True)
    parser.add_argument("-in.masksPatterns",
                        dest="masksPatterns",
                        help="",
                        nargs='+',
                        default=None,
                        required=True)
    parser.add_argument("-in.masksInitValues",
                        dest="masksInitValues",
                        help="",
                        nargs='+',
                        default=None,
                        required=True)
    parser.add_argument("-in.arboMask",
                        dest="arboMask",
                        help="",
                        default=None,
                        required=True)
    parser.add_argument("-in.GRID",
                        dest="inputGRID",
                        help="",
                        default=None,
                        required=True)
    parser.add_argument("-in.field",
                        ...
                        default=None,
                        required=True)
    parser.add_argument("-out.interpolator",
                        dest="interpolator",
                        help="",
                        default=None,
                        required=True)
    parser.add_argument("-out.projection",
                        dest="outputProjection",
                        help="",
                        default=None,
                        required=True)
    parser.add_argument("-out.resolution",
                        dest="outPixRes",
                        help="",
                        default=None,
                        required=True)
    ...
    parser.add_argument("-out.workingDirectory",
                        ...
                        help="",
                        default=None)
    parser.add_argument("-out.cmdPath",
                        dest="cmdPath",
                        help="",
                        default=None,
                        required=True)
    ...
    launchFit(args.sensorName, args.cmdPath, args.workingDirectory,
              args.outDirectory, args.outGridPath,
              args.outputGrid_tileNameField, args.outPixRes,
              args.outputProjection, args.interpolator,
              args.inputGrid_tileNameField, args.inputGRID, args.arboMask,
              args.masksInitValues, args.masksPatterns, args.arboRaster,
              args.rasterInitValue, args.rasterPatterns, args.TileFolder)
