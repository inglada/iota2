...
def getPaths(TileFolder, pattern):
    ...
    for currentS2Tile in Tiles:
        if os.path.isdir(TileFolder + "/" + currentS2Tile):
            stack = fu.FileSearch_AND(
                TileFolder + "/" + currentS2Tile, True, pattern)
            if stack:
...
def addTileToGrid(UL, UR, LR, LL, newLayer,
                  newLayerDef, tileNameField, TileName):
...
def createOutGrid(TileFolder, pattern, outputProjection,
                  tileNameField, outGridPath):
    ...
    layer = output.CreateLayer(outGridPath.split(
        "/")[-1].replace(".shp", ""), geom_type=ogr.wkbPolygon, srs=srs)
    ...
    for currentTile in paths:
        ...
        lowerL = (minX, minY)
        lowerR = (maxX, minY)
        upperL_out = converCoord(upperL, int(
            currentProjection), int(outputProjection))
        upperR_out = converCoord(upperR, int(
            currentProjection), int(outputProjection))
        lowerL_out = converCoord(lowerL, int(
            currentProjection), int(outputProjection))
        lowerR_out = converCoord(lowerR, int(
            currentProjection), int(outputProjection))
        TileName = getTileNameFromRaster(currentTile)
        addTileToGrid(upperL_out, upperR_out, lowerR_out,
                      lowerL_out, layer, layerDef, tileNameField, TileName)

    output.Destroy()


def getIntersections(outGridPath, inGridPath,
                     tileField_first, tileField_second):
    """
    OUT
    ...
    outTiles = [(outTile.GetGeometryRef().Clone(), outTile.GetField(
        tileField_first)) for outTile in layerOut]
    inTiles = [(inTile.GetGeometryRef().Clone(), inTile.GetField(
        tileField_second)) for inTile in layerIn]
...
def getAllDatasByDate(currentTile, rasterPatterns, masksPatterns,
                      arboRaster, arboMask, TileFolder, rasterInitValue, masksInitValues):

    # get all raster into the current tile and store it with init value
    buf = []
    r = []
    for currentPattern in rasterPatterns:
        r += fu.fileSearchRegEx(TileFolder + currentTile +
                                arboRaster + currentPattern)
    for currentR in r:
        buf.append((currentR, rasterInitValue))

    for currentPattern, currentMaskInit in zip(masksPatterns, masksInitValues):
        m = []
        m += fu.fileSearchRegEx(TileFolder + currentTile +
                                arboMask + currentPattern)
        for currentM in m:
...
def launchFit(sensorName, cmdPath, workingDirectory, outDirectory, outGridPath, outputGrid_tileNameField, outPixRes, outputProjection, interpolator,
              inputGrid_tileNameField, inputGRID, arboMask, masksInitValues, masksPatterns, arboRaster, rasterInitValue, rasterPatterns, TileFolder):

    createOutGrid(
        TileFolder, rasterPatterns[0], outputProjection, outputGrid_tileNameField, outGridPath)
    intersections = getIntersections(
        inputGRID, outGridPath, inputGrid_tileNameField, outputGrid_tileNameField)
    allCmd = []
    for refTile, tiles in intersections:
        ...
        for currentTile in tiles:
            datas, dates = getAllDatasByDate(
                currentTile, rasterPatterns, masksPatterns, arboRaster, arboMask, TileFolder, rasterInitValue, masksInitValues)
            for currentDatas, currentDate in zip(datas, dates):
                outFolderDate = outFolder_tile + "/" + sensorName + \
                    "_" + refTile + "_" + currentDate + "_" + currentTile
                ...
                for currentRaster in currentDatas:
                    ...
                    outName = currentRaster[0].split(
                        "/")[-1].replace(currentTile, refTile).replace(".tif", "_" + currentTile + ".tif")
                    out = folder + "/" + outName
                    cmd = "gdalwarp -t_srs EPSG:" + outputProjection + " -wo INIT_DEST=" + currentRaster[1] + " -te " + str(minX_ref) + " " + str(
                        minY_ref) + " " + str(maxX_ref) + " " + str(maxY_ref) + " -tr " + outPixRes + " -" + outPixRes + " -r " + interpolator + " " + currentRaster[0] + " " + out
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Change projection of a set of datas according to a spatial grid")

    parser.add_argument("-in.tileFolder", dest="TileFolder",
                        help="", default=None, required=True)
    parser.add_argument("-in.sensorName", dest="sensorName",
                        help="", default=None, required=True)
    parser.add_argument("-in.rasterPatterns", dest="rasterPatterns",
                        help="", nargs='+', default=None, required=True)
    parser.add_argument("-in.rasterInitValue", dest="rasterInitValue",
                        help="", default=None, required=True)
    parser.add_argument("-in.arboRaster", dest="arboRaster",
                        help="", default=None, required=True)
    parser.add_argument("-in.masksPatterns", dest="masksPatterns",
                        help="", nargs='+', default=None, required=True)
    parser.add_argument("-in.masksInitValues", dest="masksInitValues",
                        help="", nargs='+', default=None, required=True)
    parser.add_argument("-in.arboMask", dest="arboMask",
                        help="", default=None, required=True)
    parser.add_argument("-in.GRID", dest="inputGRID",
                        help="", default=None, required=True)
    parser.add_argument("-in.field", dest="inputGrid_tileNameField",
                        help="", default=None, required=True)
    parser.add_argument("-out.interpolator", dest="interpolator",
                        help="", default=None, required=True)
    parser.add_argument("-out.projection", dest="outputProjection",
                        help="", default=None, required=True)
    parser.add_argument("-out.resolution", dest="outPixRes",
                        help="", default=None, required=True)
    parser.add_argument("-out.field", dest="outputGrid_tileNameField",
                        help="", default=None, required=True)
    parser.add_argument("-out.GRID", dest="outGridPath",
                        help="", default=None, required=True)
    parser.add_argument("-out.directory", dest="outDirectory",
                        help="", default=None, required=True)
    parser.add_argument("-out.workingDirectory",
                        dest="workingDirectory", help="", default=None)
    parser.add_argument("-out.cmdPath", dest="cmdPath",
                        help="", default=None, required=True)

    args = parser.parse_args()

    launchFit(args.sensorName, args.cmdPath, args.workingDirectory, args.outDirectory, args.outGridPath, args.outputGrid_tileNameField, args.outPixRes, args.outputProjection, args.interpolator,
              args.inputGrid_tileNameField, args.inputGRID, args.arboMask, args.masksInitValues, args.masksPatterns, args.arboRaster, args.rasterInitValue, args.rasterPatterns, args.TileFolder)

    # python gridFit.py -in.tileFolder /work/S2/ -in.sensorName Sentinel2
    # -in.rasterPatterns FRE_B2.tif FRE_B3.tif FRE_B4.tif FRE_B5.tif
    # FRE_B6.tif FRE_B7.tif FRE_B8.tif FRE_B8A.tif FRE_B11.tif FRE_B12.tif
    # -in.rasterInitValue -10000 -in.arboRaster '/*/*' -in.masksPatterns
    # CLM_R1.tif EDG_R1.tif SAT_R1.tif -in.masksInitValues 0 1 0 -in.arboMask
    # '/*/MASKS/*' -in.GRID /L8_grid.shp -in.field Tile -out.interpolator
    # cubic -out.projection 2154 -out.resolution 10 -out.field tile -out.GRID
    # /S2_grid_V2.shp -out.directory /work/ -out.workingDirectory $TMPDIR
    # -out.cmdPath /reprojection.txt
