...
def addTileToGrid(
    UL, UR, LR, LL, newLayer, newLayerDef, tileNameField, TileName
):
...
def createOutGrid(
    TileFolder, pattern, outputProjection, tileNameField, outGridPath
):
    ...
    layer = output.CreateLayer(
        outGridPath.split("/")[-1].replace(".shp", ""),
    ...
    for currentTile in paths:
        ...
        lowerL = (minX, minY)
        lowerR = (maxX, minY)
        upperL_out = converCoord(
            upperL, int(currentProjection), int(outputProjection)
        )
        upperR_out = converCoord(
            upperR, int(currentProjection), int(outputProjection)
        )
        lowerL_out = converCoord(
            lowerL, int(currentProjection), int(outputProjection)
        )
        lowerR_out = converCoord(
            lowerR, int(currentProjection), int(outputProjection)
        )
...
def getIntersections(
    outGridPath, inGridPath, tileField_first, tileField_second
):
    """
    OUT
    ...
    outTiles = [
        (outTile.GetGeometryRef().Clone(), outTile.GetField(tileField_first))
        for outTile in layerOut
    ]
    inTiles = [
        (inTile.GetGeometryRef().Clone(), inTile.GetField(tileField_second))
        for inTile in layerIn
    ]
    ...
    for outTileGeom, outTile in outTiles:
        for inTileGeom, inTile in inTiles:
            ...
            if (
                intersection.GetArea() != 0.0
                and (outTile, inTile) not in AllIntersections
            ):
...
def getAllDatasByDate(
    ...
    for currentPattern in rasterPatterns:
        r += fu.fileSearchRegEx(
            TileFolder + currentTile + arboRaster + currentPattern
        )
    for currentR in r:
    ...
    for currentPattern, currentMaskInit in zip(masksPatterns, masksInitValues):
        m = []
        m += fu.fileSearchRegEx(
            TileFolder + currentTile + arboMask + currentPattern
        )
        for currentM in m:
            buf.append((currentM, currentMaskInit))
    # sort it by date
    buff = [
        (getDateFromRaster(currentRaster[0]), currentRaster)
            ...
            for currentDatas, currentDate in zip(datas, dates):
                ...
                for currentRaster in currentDatas:
                    ...
                    outName = (
                        currentRaster[0]
                        .split("/")[-1]
                        .replace(currentTile, refTile)
                        .replace(".tif", "_" + currentTile + ".tif")
                    )
                    out = folder + "/" + outName
                    cmd = (
                        "gdalwarp -t_srs EPSG:"
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-in.arboMask", dest="arboMask", help="", default=None, required=True
    )
    parser.add_argument(
        "-in.GRID", dest="inputGRID", help="", default=None, required=True
    )
    parser.add_argument(
        "-in.field",
        dest="inputGrid_tileNameField",
        help="",
        default=None,
        required=True,
    )
    ...
    parser.add_argument(
        "-out.field",
        dest="outputGrid_tileNameField",
        help="",
        default=None,
        required=True,
    )
    parser.add_argument(
        "-out.GRID", dest="outGridPath", help="", default=None, required=True
    )
    parser.add_argument(
        "-out.directory",
        dest="outDirectory",
    ...
    parser.add_argument(
        "-out.workingDirectory", dest="workingDirectory", help="", default=None
    )
    parser.add_argument(
        "-out.cmdPath", dest="cmdPath", help="", default=None, required=True
    )
    ...
    launchFit(
        ...
        args.outDirectory,
        args.outGridPath,
        args.outputGrid_tileNameField,
        args.outPixRes,
        args.outputProjection,
        args.interpolator,
        args.inputGrid_tileNameField,
        args.inputGRID,
        args.arboMask,
        args.masksInitValues,
        args.masksPatterns,
        args.arboRaster,
        args.rasterInitValue,
        args.rasterPatterns,
        args.TileFolder,
    )
