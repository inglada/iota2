...
def fitnessDateScore(dateVHR, datadir, datatype):
    ...
    """
    dateVHR = date(int(str(dateVHR)[:4]), int(
        str(dateVHR)[4:6]), int(str(dateVHR)[6:]))
    fitDate = None
    resultlist = []
    max_pixel = None
...
def coregister(
    ...
    if mode == 1 or mode == 3:
        ...
        finalOutput = os.path.join(pathWd, os.path.basename(
        ...
        shutil.move(finalOutput, insrc.replace(
            ext, ext.replace('.', '_COREG.')))
        shutil.move(finalOutput.replace(ext, '.geom'),
                    insrc.replace(ext, '_COREG.geom'))

        # Mask registration if exists
        masks = glob.glob(os.path.dirname(insrc) + os.sep +
                          'MASKS' + os.sep + '*BINARY_MASK' + ext)
        if len(masks) != 0:
            for mask in masks:
                srcClip = os.path.join(pathWd, 'tempSrcClip.tif')
                ...
                if finalMask != mask.replace(ext, ext.replace('.', '_COREG.')):
                    shutil.move(finalMask, mask.replace(
                        ext, ext.replace('.', '_COREG.')))
        ...
        if mode == 3:
            ...
            if datatype in ['S2', 'S2_S2C']:
                dates = [os.path.basename(fld).split(
                    '_')[1].split("-")[0] for fld in folders]
                ref_date = os.path.basename(insrc).split('_')[1].split("-")[0]
            elif datatype in ['L5', 'L8']:
                dates = [os.path.basename(fld).split('_')[3]
                         for fld in folders]
            ...
            for date in reversed(dates[:ref_date_ind]):
                ...
                # Mask registration if exists
                masks = glob.glob(os.path.dirname(
                    insrc) + os.sep + 'MASKS' + os.sep + '*BINARY_MASK' + ext)
                if len(masks) != 0:
                    for mask in masks:
                        srcClip = os.path.join(pathWd, 'srcClip.tif')
                        extractROIApp = OtbAppBank.CreateExtractROIApplication({"in": mask,
                ...
                if datatype in ['S2', 'S2_S2C']:
                    mtd_file = glob.glob(os.path.join(
                        os.path.dirname(insrc), '*_MTD_ALL*'))[0]
                    cloud_clear = get_S2_Tile_Cloud_Cover(mtd_file)
                    cover = get_S2_Tile_Coverage(mtd_file)
                    if cloud_clear > 0.6 and cover > 0.8:
                        clean_dates.append(date)
                elif datatype in ['L5', 'L8']:
                    mlt_file = glob.glob(os.path.join(
                        os.path.dirname(insrc), '*_MTL*'))[0]
            ...
            for date in dates[ref_date_ind + 1:]:
                ...
                if len(masks) != 0:
                    for mask in masks:
                        ...
                        ext = os.path.splitext(insrc)[1]
                        finalMask = os.path.join(pathWd, os.basename(
                            mask.replace(ext, ext.replace('.', '_COREG.'))))
                        superImposeApp = OtbAppBank.CreateSuperimposeApplication({"inr": srcClip,
                                                                                  "inm": orthoRecApp[0],
                                                                                  "out": finalMask,
...
if __name__ == "__main__":
    ...
    parser.add_argument("-insrc", dest="insrc",
                        help="Source raster", required=True)
