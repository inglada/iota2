    ...
    if mode == 1 or mode == 3:
        ...
        if mode == 3:
            ...
            for date in reversed(dates[:ref_date_ind]):
                ...
                if datatype in ["S2", "S2_S2C"]:
                    mtd_file = glob.glob(
                        os.path.join(os.path.dirname(insrc), "*_MTD_ALL*")
                    )[0]
                    ...
                    mlt_file = glob.glob(
                        os.path.join(os.path.dirname(insrc), "*_MTL*")
                    )[0]
...
if __name__ == "__main__":
    ...
    if args.mode not in [1, 2, 3]:
        sys.exit(
            "Wrong mode argument, please use the following options : 1 : simple registration ; 2 : time series registration ; 3 : time series cascade registration"
        )
