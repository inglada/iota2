...
class iota2_run(unittest.TestCase):
    ...
    def setUpClass(self):
        ...
        self.iota2_tests_directory = os.path.join(
            self.iota2_directory, "data", "test_IOTA2_scenarios"
        )
    ...
    def test_scenario_1(self):
        ...
        cfg.chain.groundTruth = os.path.join(
            self.iota2_directory,
            "data/references/sampler/D0005H0002_polygons_To_Sample.shp",
        )
        cfg.chain.nomenclaturePath = os.path.join(
            self.iota2_directory, "data/references/nomenclature.txt"
        )
        cfg.chain.runs = 1
        cfg.chain.ratio = 0.5
        cfg.chain.cloud_threshold = 1
        cfg.chain.spatialResolution = 30
        cfg.chain.colorTable = os.path.join(
            self.iota2_directory, "data/references/color.txt"
        )
