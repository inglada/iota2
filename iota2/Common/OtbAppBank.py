...
def CreateZonalStatistics(OtbParameters):
    ...
    if ZonalStatsApp is None:
        raise Exception(
            "Not possible to create 'ZonalStatistics' application, \
                         check if OTB is well configured / installed")
    ...
    if isinstance(OtbParameters["in"], str):
        ...
        inOutParam = getInputParameterOutput(OtbParameters["in"])
        ZonalStatsApp.SetParameterInputImage(
            "in", OtbParameters["in"].GetParameterOutputImage(inOutParam))
    ...
    if "inzone.vector.in" in OtbParameters:
        ...
        ZonalStatsApp.SetParameterString("inzone.vector.in",
                                         OtbParameters["inzone.vector.in"])
        ...
        if "inzone.vector.iddatafield" in OtbParameters:
            ZonalStatsApp.SetParameterString(
                "inzone.vector.iddatafield",
                OtbParameters['inzone.vector.iddatafield'])
    if "inzone.labelimage.in" in OtbParameters:
        ...
        if isinstance(OtbParameters["inzone.labelimage.in"], str):
            ZonalStatsApp.SetParameterString(
                ...
                str(OtbParameters["inzone.labelimage.in"]))
        elif isinstance(OtbParameters["inzone.labelimage.in"],
                        otb.Application):
            inOutParam = getInputParameterOutput(
                OtbParameters["inzone.labelimage.in"])
            ZonalStatsApp.SetParameterInputImage(
                "inzone.labelimage.in",
                OtbParameters["inzone.labelimage.in"].GetParameterOutputImage(
                    inOutParam))
    if "out.vector.filename" in OtbParameters:
        ZonalStatsApp.SetParameterString("out", "vector")
        ZonalStatsApp.SetParameterString("out.vector.filename",
                                         OtbParameters["out.vector.filename"])
    if "out.xml.filename" in OtbParameters:
        ZonalStatsApp.SetParameterString("out", "xml")
        ZonalStatsApp.SetParameterString("out.xml.filename",
                                         OtbParameters["out.xml.filename"])
    if "out.raster.filename" in OtbParameters:
        ZonalStatsApp.SetParameterString("out", "raster")
        ZonalStatsApp.SetParameterString("out.raster.filename",
                                         OtbParameters["out.raster.filename"])
    if "params.mean" in OtbParameters:
        ZonalStatsApp.SetParameterString("params.mean",
                                         OtbParameters["params.mean"])
    if "params.min" in OtbParameters:
        ZonalStatsApp.SetParameterString("params.min",
                                         OtbParameters["params.min"])
    if "params.max" in OtbParameters:
        ZonalStatsApp.SetParameterString("params.max",
                                         OtbParameters["params.max"])
    if "params.stdev" in OtbParameters:
        ZonalStatsApp.SetParameterString("params.stdev",
                                         OtbParameters["params.stdev"])
...
def CreateLSGRMApplication(OtbParameters):
    ...
    if isinstance(OtbParameters["in"], str):
        ...
        inOutParam = getInputParameterOutput(OtbParameters["in"])
        LSGRMApp.SetParameterInputImage(
            "in", OtbParameters["in"].GetParameterOutputImage(inOutParam))
    if "criterion" in OtbParameters:
        if str(OtbParameters["criterion"]) in ['bs', 'ed', 'fls']:
            LSGRMApp.SetParameterString("criterion",
                                        str(OtbParameters["criterion"]))
            if str(OtbParameters["criterion"] == 'bs'):
                if "criterion.bs.cw" in OtbParameters:
                    LSGRMApp.SetParameterString(
                        "criterion.bs.cw",
                        str(OtbParameters["criterion.bs.cw"]))
                if "criterion.bs.sw" in OtbParameters:
                    LSGRMApp.SetParameterString(
                        "criterion.bs.sw",
                        str(OtbParameters["criterion.bs.sw"]))
        else:
            raise Exception(
                "'criterion' parameter must be one of these value : ['bs', 'ed', 'fls']"
            )
    if "threshold" in OtbParameters:
        LSGRMApp.SetParameterString("threshold",
                                    str(OtbParameters["threshold"]))
        ...
        if str(OtbParameters["tiling"]) in ['auto', 'user', 'none']:
            ...
            if str(OtbParameters["tiling"]) == 'user':
                if "tiling.user.sizex" in OtbParameters:
                    LSGRMApp.SetParameterString(
                        "tiling.user.sizex",
                        str(OtbParameters["tiling.user.sizex"]))
                if "tiling.user.sizey" in OtbParameters:
                    LSGRMApp.SetParameterString(
                        "tiling.user.sizey",
                        str(OtbParameters["tiling.user.sizey"]))
                if "tiling.user.nfirstiter" in OtbParameters:
                    LSGRMApp.SetParameterString(
                        "tiling.user.nfirstiter",
                        str(OtbParameters["tiling.user.nfirstiter"]))
        else:
            raise Exception(
                "'tiling' parameter must be one of these value : ['auto', 'user', 'none']"
            )
