...
def computeFeatures(cfg,
    ...
    ApplicationList = [
        stack_dates, AllRefl, AllMask, datesFile_sensor, realDates
    ]

    def fields_names(sensor,
                     datesFile,
                     iota2FeatExtApp,
                     ext_Bands_Flag=None,
                     relRefl=False,
        ...
        sens_bands_names = [
            bandName
            for bandName, bandOrder in sorted(iter(
                list(sensor.bands["BANDS"].items())),
                                              key=lambda k_v: (k_v[1], k_v[0]))
        ]

        if ext_Bands_Flag:
            sens_bands_names = [
                bandName for bandName, bandNumber in list(
    ...
    S2 = Sensors.Sentinel_2(cfg.getParam('chain', 'S2Path'),
                            Opath("", create=False),
                            pathConf,
                            "",
                            createFolder=None)
    S2_S2C = Sensors.sentinel_2_s2c(cfg.getParam('chain', 'S2_S2C_Path'),
                                    Opath("", create=False),
                                    pathConf,
                                    "",
                                    createFolder=None)
    L8 = Sensors.Landsat8(cfg.getParam('chain', 'L8Path'),
                          Opath("", create=False),
                          pathConf,
                          "",
                          createFolder=None)
    L5 = Sensors.Landsat5(cfg.getParam('chain', 'L5Path'),
                          Opath("", create=False),
                          pathConf,
                          "",
                          createFolder=None)
    ...
    if (S1Data and (mode == "usually" and ds_sar_opt_fus is False)) or (
            S1Data and
        (mode != "usually" and ds_sar_opt_fus is True)) or fut.onlySAR(cfg):
    ...
    if mode == "usually":
        for gapFilling, dates, c_datesFile_sensor in zip(
            ...
            if extractBands:
                bandsToKeep = [
                    bandNumber for bandName, bandNumber in list(
                        currentSensor.keepBands.items())
                ]
                ...
                logger.debug(
                    ...
                    (comp))
            if useAddFeat:
                raw_dates = fut.getNbDateInTile(c_datesFile_sensor,
                                                display=False,
                                                raw_dates=True)
            ...
            keepduplicates = cfg.getParam('iota2FeatureExtraction',
                                          'keepduplicates')
            ...
            fields = fields_names(currentSensor,
        ...
        if userFeatPath:
            ...
            userFeat_pattern = (cfg.getParam('userFeat',
                                             'patterns')).split(",")
