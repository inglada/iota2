...
def create_dummy_rasters(missing_tiles, N, cfg):
    ...
    for tile in missing_tiles:
        classif_tile = fu.FileSearch_AND(
            classifications_dir, True, "Classif_" + str(tile))[0]
        for seed in range(N):
            dummy_raster_name = tile + "_seed_" + str(seed) + "_CompRef.tif"
            dummy_raster = final_dir + "/" + dummy_raster_name
            dummy_raster_cmd = "gdal_merge.py -ot Byte -n 0 -createonly -o " + \
                dummy_raster + " " + classif_tile
            run(dummy_raster_cmd)


def compareRef(shapeRef, shapeLearn, classif, diff,
               footprint, workingDirectory, cfg, pathWd):

    minX, maxX, minY, maxY = fu.getRasterExtent(classif)
    shapeRaster_val = workingDirectory + "/" + \
        shapeRef.split("/")[-1].replace(".sqlite", ".tif")
    shapeRaster_learn = workingDirectory + "/" + \
    ...
    # Rasterise val
    shapeRef_table_name = os.path.splitext(
        os.path.split(shapeRef)[-1])[0].lower()
    cmd = "gdal_rasterize -l " + shapeRef_table_name + " -a " + dataField + " -init 0 -tr " + str(spatialRes) + " " + str(
        spatialRes) + " " + shapeRef + " " + shapeRaster_val + " -te " + str(minX) + " " + str(minY) + " " + str(maxX) + " " + str(maxY)
    run(cmd)
    # Rasterise learn
    shapeLearn_table_name = os.path.splitext(
        os.path.split(shapeLearn)[-1])[0].lower()
    cmd = "gdal_rasterize -l " + shapeLearn_table_name + " -a " + dataField + " -init 0 -tr " + str(spatialRes) + " " + str(
        spatialRes) + " " + shapeLearn + " " + shapeRaster_learn + " -te " + str(minX) + " " + str(minY) + " " + str(maxX) + " " + str(maxY)
    run(cmd)

    # diff val
    diff_val = workingDirectory + "/" + \
        diff.split("/")[-1].replace(".tif", "_val.tif")
    cmd_val = 'otbcli_BandMath -il ' + shapeRaster_val + ' ' + classif + ' -out ' + diff_val + \
        ' uint8 -exp "im1b1==0?0:im1b1==im2b1?2:1"'  # reference identique -> 2  | reference != -> 1 | pas de reference -> 0
    run(cmd_val)
    os.remove(shapeRaster_val)

    # diff learn
    diff_learn = workingDirectory + "/" + \
        diff.split("/")[-1].replace(".tif", "_learn.tif")
    cmd_learn = 'otbcli_BandMath -il ' + shapeRaster_learn + ' ' + classif + ' -out ' + diff_learn + \
        ' uint8 -exp "im1b1==0?0:im1b1==im2b1?4:3"'  # reference identique -> 4  | reference != -> 3 | pas de reference -> 0
    run(cmd_learn)
    os.remove(shapeRaster_learn)

    # sum diff val + learn
    diff_tmp = workingDirectory + "/" + diff.split("/")[-1]
    cmd_sum = 'otbcli_BandMath -il ' + diff_val + ' ' + \
        diff_learn + ' -out ' + diff_tmp + ' uint8 -exp "im1b1+im2b1"'
    run(cmd_sum)
    os.remove(diff_val)
...
def genConfMatrix(pathClassif, pathValid, N, dataField, pathToCmdConfusion,
    ...
    for seed in range(N):
        # recherche de tout les shapeFiles par seed, par tuiles pour les
        # fusionner
    ...
    if enableCrossValidation:
        N = N - 1
    for seed in range(N):
        AllDiff = fu.FileSearch_AND(
            pathTMP, True, "_seed_" + str(seed) + "_CompRef.tif")
...
def confusion_sar_optical_parameter(iota2_dir, LOGGER=LOGGER):
    ...
    classifications = fu.FileSearch_AND(
        classifications_dir,
        True,
        "Classif",
        "model",
    ...
    for vector in vectors:
        ...
        if len(fu.getFieldElement(vector, driverName="ESRI Shapefile",
                                  field=fields[0], mode="all", elemType="str")):
    ...
    for group in groups_param_buff:
        if len(group) != 3:
            err_message = "all parameter to use Dempster-Shafer fusion, not found : {}".format(
                group)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-path.classif",
        help="path to the folder which contains classification images (mandatory)",
        dest="pathClassif",
        required=True)
    parser.add_argument(
        "-path.valid",
    ...
    parser.add_argument(
        ...
        help="path where all confusion cmd will be stored in a text file(mandatory)",
        required=True)
    parser.add_argument(
        "--wd",
        dest="pathWd",
