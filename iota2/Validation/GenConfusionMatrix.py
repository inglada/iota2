...
# =========================================================================
"""
Generate Confusion Matrix Modules
"""
import argparse
import os
import shutil
...
def create_dummy_rasters(missing_tiles, N, cfg):
    ...
    classifications_dir = os.path.join(cfg.getParam('chain', 'outputPath'),
                                       "classif")
    final_dir = os.path.join(cfg.getParam('chain', 'outputPath'), "final",
                             "TMP")
...
def compareRef(shapeRef, shapeLearn, classif, diff, footprint,
               workingDirectory, cfg, pathWd):

    minX, maxX, minY, maxY = fu.getRasterExtent(classif)
    shapeRaster_val = workingDirectory + "/" + shapeRef.split("/")[-1].replace(
        ".sqlite", ".tif")
    shapeRaster_learn = workingDirectory + "/" + shapeLearn.split(
    ...
    cmd = "gdal_rasterize -l " + shapeRef_table_name + " -a " + dataField + " -init 0 -tr " + str(
        spatialRes) + " " + str(
            spatialRes
        ) + " " + shapeRef + " " + shapeRaster_val + " -te " + str(
            minX) + " " + str(minY) + " " + str(maxX) + " " + str(maxY)
    run(cmd)
    #Rasterise learn
    shapeLearn_table_name = os.path.splitext(
        os.path.split(shapeLearn)[-1])[0].lower()
    cmd = "gdal_rasterize -l " + shapeLearn_table_name + " -a " + dataField + " -init 0 -tr " + str(
        spatialRes) + " " + str(
            spatialRes
        ) + " " + shapeLearn + " " + shapeRaster_learn + " -te " + str(
            minX) + " " + str(minY) + " " + str(maxX) + " " + str(maxY)
    run(cmd)
    ...
    #diff val
    diff_val = workingDirectory + "/" + diff.split("/")[-1].replace(
        ".tif", "_val.tif")
    ...
    #diff learn
    diff_learn = workingDirectory + "/" + diff.split("/")[-1].replace(
        ".tif", "_learn.tif")
    ...
    for seed in range(N):
        ...
        for tile in AllTiles:
            ...
            cmd = 'otbcli_ComputeConfusionMatrix -in {}/Classif_Seed_{}.tif -out {}/{}_seed_{}.csv -ref.vector.field {} -ref vector -ref.vector.in {}'.format(
                pathClassif, seed, pathDirectory, tile, seed,
                dataField.lower(), valTile)
            AllCmd.append(cmd)
            ...
            compareRef(valTile, learnTile, classif, diff, footprint,
                       workingDirectory, cfg, pathWd)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-path.classif",
        help=
        "path to the folder which contains classification images (mandatory)",
        dest="pathClassif",
        required=True)
    parser.add_argument(
        "-path.valid",
        help=
        "path to the folder which contains validation samples (with priority) (mandatory)",
        dest="pathValid",
        required=True)
    ...
    parser.add_argument(
        ...
        help=
        "path where all confusion cmd will be stored in a text file(mandatory)",
        required=True)
