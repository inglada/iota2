...
def compareRef(
    ...
    cmd_val = (
        ...
        + " -out "
        + diff_val
        + ' uint8 -exp "im1b1==0?0:im1b1==im2b1?2:1"'
    ...
    cmd_learn = (
        ...
        + " -out "
        + diff_learn
        + ' uint8 -exp "im1b1==0?0:im1b1==im2b1?4:3"'
...
def genConfMatrix(
    pathClassif, pathValid, N, dataField, pathToCmdConfusion, cfg, pathWd
):
    ...
    for seed in range(N):
        ...
        for tile in AllTiles:
            ...
            valTile = fu.FileSearch_AND(
                pathValid, True, tile, "_seed_" + str(seed_val) + "_val.sqlite"
            )[0]
            learnTile = fu.FileSearch_AND(
                pathValid, True, tile, "_seed_" + str(seed) + "_learn.sqlite"
            )[0]
    ...
    for seed in range(N):
        ...
        if pathWd:
            shutil.copy(
                workingDirectory + "/diff_seed_" + str(seed) + ".tif",
