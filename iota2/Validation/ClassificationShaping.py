...
def BuildNbVoteCmd(classifTile, VoteMap):
    ...
    cmd = 'otbcli_BandMath -il ' + imgs + ' -out ' + \
        VoteMap + ' -exp "' + expVote + '"'
    return cmd


def BuildConfidenceCmd(finalTile, classifTile, confidence,
                       OutPutConfidence, fact=100, pixType="uint8"):
    ...
    for i in range(len(classifTile)):
        exp.append("(im" + str(i + 2) + "b1==0?0:im1b1!=im" + str(i + 2) +
                   "b1?1-im" + str(i + 2 + N) + "b1:im" + str(i + 2 + N) + "b1)")
    # expConfidence="im1b1==0?0:("+"+".join(exp)+")/im"+str(2+2*N)+"b1"
    expConfidence = "im1b1==0?0:(" + "+".join(exp) + \
        ")/" + str(len(classifTile))

    All = classifTile + confidence
    All = " ".join(All)

    cmd = 'otbcli_BandMath -ram 5120 -il ' + finalTile + ' ' + All + ' -out ' + \
        OutPutConfidence + ' ' + pixType + ' -exp "' + \
        str(fact) + '*(' + expConfidence + ')"'
...
def proba_map_fusion(proba_map_list, ram=128,
                     working_directory=None, logger=logger):
    ...
    exp = "({}) dv {}".format("+".join(["im{}".format(i + 1)
                                        for i in range(len(proba_map_list))]), len(proba_map_list))
    proba_map_fus_path = os.path.join(proba_map_fus_dir, proba_map_fus_name)
    if working_directory:
        proba_map_fus_path = os.path.join(
            working_directory, proba_map_fus_name)
    logger.info(
        "Fusion of probality maps : {} at {}".format(
            proba_map_list,
            proba_map_fus_path))
            ...
            if shapeRegion is None:
                if classifMode == "separate":
                    confidence_pattern = os.path.join(
                        pathToClassif, "{}*model*confidence_seed_{}*.tif".format(tuile, seed))
                    if ds_sar_opt:
                        confidence_pattern = os.path.join(
                            pathToClassif, "{}*model*confidence_seed_{}*_DS.tif".format(tuile, seed))
                    confidence = fu.fileSearchRegEx(confidence_pattern)
                    globalConf = tmpClassif + "/" + tuile + \
                        "_GlobalConfidence_seed_" + str(seed) + ".tif"
                    globalConf_f = pathTest + "/final/TMP/" + tuile + \
                        "_GlobalConfidence_seed_" + str(seed) + ".tif"
                    cmd = 'otbcli_BandMath -il ' + \
                        confidence[0] + ' -out ' + globalConf + \
                        ' uint8 -exp "100*im1b1"'
                    run(cmd)
                ...
                if classifMode != "separate":
                    classifTile = fu.fileSearchRegEx(
                        ...
                        "*model_*f*_seed_" +
                        str(seed) +
                        suffix)  # tmp tile (produce by each classifier, without nodata)
                    splitModel = []
                    for classif in classifTile:
                        model = classif.split(
                            "/")[-1].split("_")[3].split("f")[0]
                        try:
                    ...
                    confidence_all = fu.fileSearchRegEx(
                        pathToClassif + "/" + tuile + "*model_*_confidence_seed_" + str(seed) + suffix)
                    ...
                    for model in splitModel:
                        classifTile = fu.fileSearchRegEx(
                            ...
                            str(seed) +
                            suffix)  # tmp tile (produce by each classifier, without nodata)
                        finalTile = pathToClassif + "/Classif_" + tuile + \
                            "_model_" + model + "_seed_" + str(seed) + ".tif"
                        if ds_sar_opt:
                            finalTile = pathToClassif + "/Classif_" + tuile + \
                                "_model_" + model + "_seed_" + \
                                str(seed) + "_DS.tif"
                        ...
                        OutPutConfidence = tmpClassif + "/" + tuile + "_model_" + \
                            model + "_confidence_seed_" + str(seed) + ".tif"
                        if ds_sar_opt:
                            OutPutConfidence = tmpClassif + "/" + tuile + "_model_" + \
                                model + "_confidence_seed_" + \
                                str(seed) + "_DS.tif"
                        cmd = BuildConfidenceCmd(
                            finalTile,
                            classifTile,
                            confidence,
                            OutPutConfidence,
                    ...
                    exp1 = "+".join(["im" + str(i + 1) +
                                     "b1" for i in range(len(splitConfidence))])
                    exp2 = "+".join(["(100*im" + str(j + 1) + "b1)" for j in np.arange(i + 1, i + 1 + len(
                        confidence_withoutSplit))])  # -> confidence from NO-splited models are from 0 to 1
                    if not splitConfidence:
                        exp2 = "+".join(["100*im" + str(j + 1) +
                                         "b1" for j in range(len(confidence_withoutSplit))])
                    if exp1 and exp2:
                        exp = exp1 + "+" + exp2
                    ...
                    OutPutConfidence = tmpClassif + "/" + tuile + \
                        "_GlobalConfidence_seed_" + str(seed) + ".tif"
                    ...
                    confidence = fu.fileSearchRegEx(
                        pathToClassif + "/" + tuile + "*model*confidence_seed_" + str(seed) + suffix)
                    exp = "+".join(["im" + str(i + 1) +
                                    "b1" for i in range(len(confidence))])
...
def ClassificationShaping(pathClassif, pathEnvelope, pathImg, fieldEnv, N,
    ...
    ds_sar_opt = cfg.getParam('argTrain', 'dempster_shafer_SAR_Opt_fusion')
    proj = cfg.getParam('GlobChain', 'proj').split(":")[-1]
    AllTile = list(set([classif.split("_")[1] for classif in fu.FileSearch_AND(
    ...
    if shapeRegion and classifMode == "fusion":
        old_classif = fu.fileSearchRegEx(
            pathTest + "/classif/Classif_*_model_*f*_seed_" + suffix + ".tif")
    ...
    for seed in range(N):
        ...
        if proba_map_flag:
            proba_map_list = fu.fileSearchRegEx(
                pathTest + "/classif/PROBAMAP_*_model_*_seed_" + str(seed) + suffix + ".tif")
        ...
        if classifMode == "separate" or shapeRegion:
            AllClassifSeed = fu.FileSearch_AND(
                pathClassif, True, ".tif", "Classif", "seed_" + str(seed))
            if ds_sar_opt:
                AllClassifSeed = fu.FileSearch_AND(
                    pathClassif, True, ".tif", "Classif", "seed_" + str(seed), "DS.tif")
            ind = 1
            ...
            if ds_sar_opt:
                AllClassifSeed = fu.FileSearch_AND(
                    pathClassif, True, "_FUSION_NODATA_seed" + str(seed) + "_DS.tif")
            ind = 0
        ...
        for tile, paths in sort:
            ...
            cmd = 'otbcli_BandMath -il ' + allCl + '-out ' + \
                path_Cl_final + ' ' + pixType + ' -exp "' + exp + '"'
            run(cmd)

            tileConfidence = pathOut + "/TMP/" + tile + \
                "_GlobalConfidence_seed_" + str(seed) + ".tif"
            confidence[seed].append(tileConfidence)
            cloudTile = fu.FileSearch_AND(
                featuresPath + "/" + tile, True, "nbView.tif")[0]
            ...
            cloudTilePriority_StatsOK = pathTest + \
                "/final/TMP/" + tile + "_Cloud_StatsOK.tif"
            ...
            if not os.path.exists(cloudTilePriority):
                cmd_cloud = 'otbcli_BandMath -il ' + cloudTile + ' ' + ClassifTile + \
                    ' -out ' + cloudTilePriority_tmp + ' int16 -exp "im2b1>0?im1b1:0"'
                run(cmd_cloud)
                if outputStatistics:
                    cmd_cloud = 'otbcli_BandMath -il ' + cloudTile + ' ' + ClassifTile + \
                        ' -out ' + cloudTilePriority_tmp_StatsOK + ' int16 -exp "im2b1>0?im1b1:-1"'
                    run(cmd_cloud)
                    if pathWd:
                        shutil.copy(
                            cloudTilePriority_tmp_StatsOK,
    ...
    for seed in range(N):
        ...
        if proba_map_flag:
            proba_map_mosaic = os.path.join(
                assembleFolder, "ProbabilityMap_seed_{}.tif".format(seed))
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-path.img",
        help="path to the folder which contains images (mandatory)",
        dest="pathImg",
        required=True)
    parser.add_argument(
        "-field.env",
        help="envelope's field into shape(mandatory)",
        dest="fieldEnv",
        required=True)
    parser.add_argument(
        "-N",
