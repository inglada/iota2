...
def addClassDescription(classDescriptions, file_JS):
    ...
    stringList = [
        "\t" + str(classNumber) + ':"' + label + '"'
        for classNumber, label, r, g, b in classDescriptions
    ]
    stringDico = ",\n".join(stringList)
...
def generateOneClassificationHTML(cfgFile):
    ...
    file_JS.write(
        CS.var_mapLayers_overlays
        + ",".join([fig_id, "validity", "confidence"])
        + "]}),\n"
    )
    file_JS.write("]});\nurl = 'filter.geojson';\n")
...
def generateMultiClassificationHTML(cfgFile):
    ...
    Allfig_id = []
    for resultat, title, color, desc, out in zip(
        AllRes, AllTitle, AllColor, AllClassesDescriptions, AllOut
    ):
...
\n\
    ...
    for i in range(len(Allfig_id)):
        file_HTML.write(
            '\t<div id="'
            + Allfig_id[i]
            + '" style="float: left;display:none;width:50%"></div>\n'
        )
...
def generateHTML_code(cfgFile):
    ...
    if outputFormat == "multiClassifications":
        generateMultiClassificationHTML(cfgFile)
    elif outputFormat == "oneClassification":
        generateOneClassificationHTML(cfgFile)
